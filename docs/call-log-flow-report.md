...
## Optional: no-form auto-save (UI calls)

- If you don’t want the “Call Detected” form modal after calls started from Enquiry/FollowUp screens, enable:
  - `.env` → `EXPO_PUBLIC_CALL_AUTO_SAVE=true`
- With this flag, those screens will auto-save the call log on `CALL_ENDED` and won’t open `PostCallModal`.
