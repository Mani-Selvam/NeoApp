...
const logAction = async (req, action, metadata = {}, category = "admin_action") => {
...
exports.getDashboard = async (req, res) => {
  try {
    const [
      ...
      CompanySubscription.countDocuments({ status: { $in: ["Active", "Trial"] } }),
      Enquiry.countDocuments(),
      CallLog.countDocuments(),
      CompanySubscription.aggregate([
        { $match: { status: { $in: ["Active", "Trial", "Cancelled", "Expired"] } } },
      ...
      CompanySubscription.aggregate([
        {
          $project: {
            label: {
              $dateToString: { format: "%Y-%m", date: { $ifNull: ["$startDate", "$createdAt"] } },
            },
            amount: "$finalPrice",
          },
        },
      ...
      User.aggregate([
        ...
        { $project: { label: { $dateToString: { format: "%Y-%m", date: "$createdAt" } } } },
    ...
    await logAction(req, "SUPERADMIN_DASHBOARD_ERROR", { message: error.message }, "error");
...
exports.getCompanies = async (_req, res) => {
  try {
    ...
    const plans = await Plan.find({ _id: { $in: planIds } }).select("_id name code").lean();
    const planMap = new Map(plans.map((p) => [p._id.toString(), p]));

    const response = companies.map((c) => ({
      ...c,
      plan: c.planId ? (planMap.get(c.planId.toString())?.name || "-") : "-",
      planCode: c.planId ? (planMap.get(c.planId.toString())?.code || "-") : "-",
    }));
...
exports.updateCompanyStatus = async (req, res) => {
    try {
      ...
      await logAction(req, "COMPANY_STATUS_UPDATED", { companyId: req.params.companyId, status });
...
exports.deleteCompany = async (req, res) => {
  try {
    ...
    await logAction(req, "COMPANY_DELETED", { companyId: req.params.companyId });
...
exports.respondSupportTicket = async (req, res) => {
  try {
    const { responseMessage, close = false, activateCompany = false } = req.body || {};

    if (!responseMessage || !String(responseMessage).trim()) {
      return res.status(400).json({ success: false, message: "responseMessage is required" });
    }
    ...
    res.status(500).json({ success: false, message: "Failed to respond to support ticket" });
  }
};
...
exports.updateUserStatus = async (req, res) => {
  try {
    ...
    await logAction(req, "USER_STATUS_UPDATED", { userId: req.params.userId, status });
...
exports.resetUserPassword = async (req, res) => {
  try {
    ...
    if (!password || String(password).length < requiredLen) {
      return res.status(400).json({ success: false, message: `Password must be at least ${requiredLen} characters` });
    }
    ...
    await logAction(req, "USER_PASSWORD_RESET", { userId: req.params.userId });
...
exports.updateWorkspaceSettings = async (req, res) => {
  try {
    ...
    const workspace = await setWorkspaceSettings({ name, supportEmail, defaultTimezone });
    await logAction(req, "WORKSPACE_SETTINGS_UPDATED", { keys: Object.keys(req.body || {}) });
    return res.json({ success: true, workspace });
  } catch (error) {
    return res.status(400).json({ success: false, message: error.message });
  }
...
exports.updateSecurityPolicySettings = async (req, res) => {
  try {
    ...
    await logAction(req, "SECURITY_POLICY_UPDATED", { keys: Object.keys(req.body || {}) });
...
exports.updateRazorpaySettings = async (req, res) => {
  try {
    const { keyId, keySecret, webhookSecret, syncEnv = true } = req.body || {};
...
exports.createCoupon = async (req, res) => {
  try {
    ...
    payload.globalUsageLimit = Math.max(1, Number(payload.globalUsageLimit || 1));
    payload.perCompanyUsageLimit = Math.max(1, Number(payload.perCompanyUsageLimit || 1));
...
exports.updateCoupon = async (req, res) => {
  try {
    ...
    if (payload.code) payload.code = String(payload.code).toUpperCase();
    if (payload.usageLimit && !payload.globalUsageLimit) payload.globalUsageLimit = payload.usageLimit;
    if (payload.globalUsageLimit != null) payload.globalUsageLimit = Math.max(1, Number(payload.globalUsageLimit));
    if (payload.perCompanyUsageLimit != null) payload.perCompanyUsageLimit = Math.max(1, Number(payload.perCompanyUsageLimit));
    if (payload.globalUsageLimit != null) payload.usageLimit = payload.globalUsageLimit;
    if (payload.appliesToAllCompanies) {
      payload.applicableCompanies = [];
    }
...
exports.deleteCoupon = async (req, res) => {
  try {
    ...
    const linkedSubscriptions = await CompanySubscription.countDocuments({ couponId: req.params.couponId });
...
exports.upsertOverride = async (req, res) => {
  try {
    ...
    await logAction(req, "COMPANY_OVERRIDE_UPSERTED", { companyId, updates });
    res.json(override);
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
...
exports.deleteOverride = async (req, res) => {
  try {
    const deleted = await CompanyPlanOverride.findByIdAndDelete(req.params.overrideId);
    if (!deleted) return res.status(404).json({ success: false, message: "Override not found" });
...
exports.deleteSubscription = async (req, res) => {
  try {
    const deleted = await CompanySubscription.findByIdAndDelete(req.params.subscriptionId).lean();
...
exports.assignSubscription = async (req, res) => {
  try {
    ...
    if (!companyId || !planId) {
      return res.status(400).json({ success: false, message: "companyId and planId are required" });
    }

    const plan = await Plan.findById(planId).lean();
    if (!plan || !plan.isActive) return res.status(400).json({ success: false, message: "Invalid or inactive plan" });
    ...
    if (couponCode) {
      ...
      const companyUsedCount = getCompanyCouponUsedCount(coupon, companyId);
      ...
      if (coupon.applicablePlans?.length && !coupon.applicablePlans.some((id) => id.toString() === planId)) {
        return res.status(400).json({ success: false, message: "Coupon not applicable to selected plan" });
      }
      if (
        !coupon.appliesToAllCompanies &&
        coupon.applicableCompanies?.length &&
        !coupon.applicableCompanies.some((id) => id.toString() === companyId)
      ) {
        return res.status(400).json({ success: false, message: "Coupon not applicable to selected company" });
      }
    }
    ...
    if (coupon) {
      ...
      coupon.companyUsageMap.set(String(companyId), getCompanyCouponUsedCount(coupon, companyId) + 1);
      await coupon.save();
    }
...
exports.getEffectivePlanByCompany = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to resolve effective plan" });
  }
};

exports.getRevenue = async (_req, res) => {
  try {
    const [monthlyRevenueAgg, yearlyRevenueAgg, activeSubscriptions, cancelledSubscriptions, chartAgg] = await Promise.all([
      CompanySubscription.aggregate([
        { $match: { startDate: { $gte: startOfMonth } } },
        { $group: { _id: null, value: { $sum: "$finalPrice" } } },
      ]),
