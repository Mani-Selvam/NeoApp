...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const callLogDeleteOr = [{ staffId }];
      const callSessionDeleteOr = [{ staffId }];
      ...
      if (relatedEnquiryIds.length > 0) {
        ...
        callLogDeleteOr.push({ enquiryId: { $in: relatedEnquiryIds } });
        callSessionDeleteOr.push({ enquiryId: { $in: relatedEnquiryIds } });
      ...
      await Promise.all([
        ...
        CallLog.deleteMany({ $or: callLogDeleteOr }),
        CallSession.deleteMany({ $or: callSessionDeleteOr }),
