...
// Client sends this after getting Expo push token
router.post("/register-push-token", verifyToken, async (req, res) => {
    try {
        ...
        console.log(
            `[PushNotif] ✓ Push token registered for user ${userId}: ${pushToken.substring(
                0,
                30,
            )}...`,
        );
        ...
        res.json({
            success: true,
            message: "Push token registered for closed-app notifications",
        });
    } catch (error) {
        console.error(
            "[PushNotif] Error registering push token:",
            error?.message,
        );
