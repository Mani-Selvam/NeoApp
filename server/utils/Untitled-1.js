...
const getStatusLabel = (status) => {
  ...
  return normalized === "Contacted" ? "Connected" : normalized;
};
...
const DetailView = ({
  ...
  const detailPanHandlers = tabIdx === 4 ? {} : swipePan.panHandlers;
  const whatsappEdgePan = useRef(
    PanResponder.create({
      onStartShouldSetPanResponder: () =>
        tabRef.current === 4 && !tabGestureLockedRef.current,
      onStartShouldSetPanResponderCapture: () =>
        tabRef.current === 4 && !tabGestureLockedRef.current,
      onMoveShouldSetPanResponder: (_, g) => {
        if (tabRef.current !== 4 || tabGestureLockedRef.current) return false;
      ...
      onPanResponderRelease: (_, g) => {
        if (tabGestureLockedRef.current || tabRef.current !== 4) return;
        if (g.dx < -56) {
          goToTab(5);
          return;
        }
          ...
          {tabIdx === 0 && (
            ...
            >
              <LinearGradient
                ...
                style={DV.statusSummaryCard}
              >
                <View
                  style={[DV.statusSummaryIcon, { backgroundColor: sCfg.bg }]}
                >
                  <Ionicons name="flag-outline" size={18} color={sCfg.color} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={DV.statusSummaryLabel}>Current Status</Text>
                  <Text style={[DV.statusSummaryValue, { color: sCfg.color }]}>
                    {getStatusLabel(enquiry.status)}
                  </Text>
                  <Text style={DV.statusSummarySub}>
                    Next action tabs and follow-up options are based on this
                    status.
                  </Text>
                </View>
              </LinearGradient>
              <View style={{ gap: 8 }}>
                {[
                  ...
                  {
                    label: "Status",
                    value: getStatusLabel(enquiry.status) || "-",
                    icon: "flag-outline",
                  },
                  {
                    label: "Last Contact",
              ...
              {selectedEnquiry && (
                <View style={{ gap: 12 }}>
                  ...
                  <View style={DV.followupContext}>
                    ...
                    >
                      <Text
                        style={[DV.inlineStatusText, { color: sCfg.color }]}
                      >
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={["top"]}>
      ...
      {detailEnquiry && (
        <View style={StyleSheet.absoluteFill}>
          <DetailView
            key={`detail-${detailEnquiry?._id || detailEnquiry?.enqNo || "enquiry"}-${detailInitialTab}`}
