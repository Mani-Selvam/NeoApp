...
const VOICE_NAV_TARGETS = [
    ...
    },
    {
        route: "FollowUp",
        label: "FollowUp",
        icon: "calendar-outline" as React.ComponentProps<typeof Ionicons>["name"],
    },
    {
        route: "Communication",
        label: "Task",
        icon: "chatbubbles-outline" as React.ComponentProps<typeof Ionicons>["name"],
    },
    {
        route: "Report",
        label: "Report",
        icon: "bar-chart-outline" as React.ComponentProps<typeof Ionicons>["name"],
    },
];
...
function MainTabNavigator() {
    ...
    // - On iOS: use standard safe-area bottom inset, default to a minimum of 10.
    // - On Android: if effectiveBottomInset > 0 (gesture navigation or translucent 3-button bar), use the exact OS value.
    // - Otherwise, use a safe default of 8.
    let bottomPad = Platform.OS === "ios"
        ? Math.max(insets.bottom || 0, 10)
        : (effectiveBottomInset > 0 ? effectiveBottomInset : 8);
    ...
    return (
        <Tab.Navigator
            ...
            <Tab.Screen
                ...
                component={WhatsAppTemplateAccessForTabs}
                options={hiddenTabOptions}
            />
            <Tab.Screen
                name="BulkWhatsAppScreen"
                component={BulkWhatsAppAccessForTabs}
                options={hiddenTabOptions}
            />
            <Tab.Screen
                name="ProfileScreen"
...
function GlobalVoiceAssistantFab({
    currentRouteName,
}: {
    currentRouteName: string;
}) {
    const insets = useSafeAreaInsets();
    const { billingInfo, showUpgradePrompt } = useAuth();
    ...
    const activeTarget = VOICE_NAV_TARGETS.find((item) => {
        if (item.route === "Enquiry") {
            return /enquiry|addenquiry/i.test(currentRouteName);
        }
        if (item.route === "FollowUp") {
            return /follow/i.test(currentRouteName);
        }
        if (item.route === "Communication") {
            return /communication|chat/i.test(currentRouteName);
        }
        if (item.route === "Report") {
            return /report/i.test(currentRouteName);
        }
        return false;
    });
    ...
    const openAssistant = () => {
        setNavOpen(false);
        if (!hasPlanFeature(billingInfo?.plan, "voice_assistant")) {
            showUpgradePrompt(
                buildFeatureUpgradeMessage("voice_assistant", "Voice Assistant"),
            );
            return;
        }
    ...
    const bottom = Math.max(insets.bottom || 0, 12) + 74;
        ...
        <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
            ...
            <Animated.View
                ...
                <TouchableOpacity
                    ...
                    style={globalVoiceStyles.fabButton}>
                    <LinearGradient
                        colors={["#8B5CF6", "#06B6D4"]}
                        style={globalVoiceStyles.fabInner}>
                        <Ionicons
                            name="mic"
                            size={24}
                            color="#FFFFFF"
...
const WhatsAppTemplateStaffRestrictedScreen =
    makeStaffRestricted("WhatsApp Template");
const WhatsAppTemplatePlanAccessScreen = createPlanRestrictedScreen(
    WhatsAppTemplateScreen as any,
    "whatsapp",
    "WhatsApp Template",
);
const WhatsAppTemplateAccessForTabs = (props: any) => {
    const { user } = useAuth();
    const isStaffUser = String(user?.role || "").toLowerCase() === "staff";
    const Component = isStaffUser
        ? WhatsAppTemplateStaffRestrictedScreen
        : WhatsAppTemplatePlanAccessScreen;
    return <Component {...props} />;
};
const BulkWhatsAppPlanAccessScreen = createPlanRestrictedScreen(
    BulkWhatsAppScreen as any,
    "whatsapp",
    "Bulk WhatsApp",
);
const BulkWhatsAppAccessForTabs = (props: any) => {
    const { user } = useAuth();
    const isStaffUser = String(user?.role || "").toLowerCase() === "staff";
    const Component = isStaffUser
        ? WhatsAppTemplateStaffRestrictedScreen // reuse same restriction screen
        : BulkWhatsAppPlanAccessScreen;
    return <Component {...props} />;
};
...
export default function AppNavigator() {
    ...
    return (
        <NavigationContainer
                ...
                <TouchableOpacity
                    ...
                    <TouchableOpacity
                        ...
                        <View style={{
                            ...
                            <Ionicons name="rocket-outline" size={32} color="#4F46E5" />
                        </View>
                        <Text
                            style={{
                                fontSize: 22,
                            ...
                            {billingPrompt?.title || "Upgrade to Premium"}
                        </Text>
                        <Text
                            style={{
                                marginTop: 12,
                            ...
                            {billingPrompt?.message ||
                                "Unlock the full potential of NeoGroww. Upgrade your plan to access this exclusive feature."}
                        </Text>
                        <View
                            style={{
                                    ...
                                    if (!isStaffUser) {
                                        if (navigationRef.isReady()) {
                                            try {
                                                ...
                                                console.warn("Navigation to PricingScreen failed:", e);
                                            }
                                        }
                                    }
                                }}
            ...
            {isLoggedIn ? (
                <GlobalVoiceAssistantFab currentRouteName={currentRouteName} />
            ) : null}
        </NavigationContainer>
    );
}
...
const globalVoiceStyles = StyleSheet.create({
    fabWrap: {
        position: "absolute",
        right: 18,
        width: 58,
