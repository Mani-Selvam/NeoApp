...
import {
    ...
    getMissedFollowUpsSummaryTexts,
...
const CHANNEL_IDS = {
    ...
    security_alerts: "security_alerts_v1",
...
const MINUTE_NOTIFICATION_CHANNELS = (() => {
    ...
    const minuteSoundFile = (typeKey, langKey, marker) => {
        if (marker === 60 || marker === "60") {
            return langKey === "ta" ? "tamil_general_followup_60min" : "general_followup_60min";
        }
        const map = {
            followups: {
                en: {
    ...
    for (const t of Object.keys(typeLabels)) {
        for (const l of Object.keys(langLabels)) {
            for (const m of [60, 5, 4, 3, 2, 1]) {
                const key = `${t}_${m}min_${l}`;
                meta[key] = mk(
                    `${typeLabels[t]} (${m === 60 ? '60' : m} min) ${langLabels[l]}`,
...
// ─── Initialize ───────────────────────────────────────────────────────────────
export const initializeNotifications = async (forcePrompt = false) => {
    try {
        if (Platform.OS === "web") {
        ...
        if (status !== "granted") {
            const shouldPrompt =
                forcePrompt ||
                (!hasExplained && existingPermission.canAskAgain !== false);
        ...
        console.log(`[NotifSvc] Permission status: ${status}`);

        if (status !== "granted") {
            console.log("[NotifSvc] Silent initialization completed (no permission)");
            return false;
        }

        console.log("[NotifSvc] ✓ Permission granted - setting up channels");
...
export const notifyMissedFollowUpsSummary = async (followUps = []) => {
    try {
        ...
        const lang = await getNotificationVoiceLanguage();
        const texts = getMissedFollowUpsSummaryTexts({ lang, count });
