...
export default function EmailScreen({ navigation, route, embedded = false }) {
    ...
    const groupedLogs = useMemo(() => {
        if (tab !== "sent") return logs;
        const groups = {};
        logs.forEach((log) => {
            const recipient = String(log.to || "").trim().toLowerCase();
            if (!recipient) return;
            if (!groups[recipient]) {
                groups[recipient] = {
                    key: recipient,
                    to: log.to,
                    logs: [],
                    latestSentAt: log.sentAt || log.createdAt,
                    latestSubject: log.subject || "–",
                };
            }
            groups[recipient].logs.push(log);
            const date = new Date(log.sentAt || log.createdAt);
            const latest = new Date(groups[recipient].latestSentAt);
            if (date > latest) {
                groups[recipient].latestSentAt = log.sentAt || log.createdAt;
                groups[recipient].latestSubject = log.subject || "–";
            }
        });
        return Object.values(groups).sort((a, b) => {
            return new Date(b.latestSentAt) - new Date(a.latestSentAt);
        });
    }, [logs, tab]);
    ...
    const onSend = async () => {
        try {
            ...
            if (isBulk) {
                Alert.alert(
                    "Confirm Bulk Send",
                    "Are you sure you want to send this email to all of your enquiries?",
                    [
                        { text: "Cancel", style: "cancel" },
                        {
                            ...
                            onPress: async () => {
                                try {
                                    setLoading(true);
                                    setBulkProgress({ current: 0, total: 0, success: 0, failed: 0, done: false });
                                    
                                    let page = 1;
                                    let emails = [];
                                    while (true) {
                                        const res = await getAllEnquiries(page, 100);
                                        const data = Array.isArray(res) ? res : res?.data || [];
                                        if (!data.length) break;
                                        data.forEach((enq) => {
                                            if (enq.email && enq.email.trim()) {
                                                emails.push(enq.email.trim());
                                            }
                                        });
                                        if (Array.isArray(res) || !res?.pagination?.pages || page >= res.pagination.pages) {
                                            break;
                                        }
                                        page++;
                                    }
                                    ...
                                    if (uniqueEmails.length === 0) {
                                        ...
                                        setLoading(false);
                                        return Alert.alert("No emails", "No active enquiries have registered email addresses.");
                                    }

                                    const total = uniqueEmails.length;
                                    setBulkProgress({ current: 0, total, success: 0, failed: 0, done: false });
                                    ...
                                    Alert.alert("Bulk Send Error", e.message || "Failed to process bulk send");
                                } finally {
                                    setLoading(false);
                                }
                            }
    ...
    const renderGroupedLogRow = ({ item }) => {
        const totalSent = item.logs.length;
        return (
            <TouchableOpacity
                activeOpacity={0.8}
                    ...
                    <View
                        ...
                        <View
                            ...
                            <Text
                                ...
                                {totalSent} {totalSent === 1 ? "email" : "emails"}
                            </Text>
                        </View>
                    </View>
                    <Text style={S.rowSub} numberOfLines={1}>
                        Latest: {item.latestSubject}
                    </Text>
                    <Text style={S.rowMeta}>
                        Last sent: {formatDateTime(item.latestSentAt)}
                    </Text>
                </View>
                <Ionicons
                    name="chevron-forward"
        ...
        <View style={S.root}>
            ...
            {tab === "compose" ? (
                <KeyboardAvoidingView
                    ...
                    <ScrollView
                        ...
                        {selectedTemplate ? (
                            <View style={S.activeTemplateCard}>
                                <LinearGradient
                                    colors={["rgba(212,175,55,0.08)", "rgba(212,175,55,0.02)"]}
                                    start={{ x: 0, y: 0 }}
                                    end={{ x: 1, y: 1 }}
                                    style={S.activeTemplateGrad}>
                                    
                                    <View style={S.activeTemplateIconBox}>
                                        <Ionicons
                                            name="sparkles"
                                            size={16}
                                            color={T.gold}
                                    ...
                                    <View style={S.activeTemplateContent}>
                                        <Text style={S.activeTemplateBadge}>ACTIVE TEMPLATE</Text>
                                        <Text style={S.activeTemplateTitle} numberOfLines={1}>
                                            {selectedTemplate.name}
                                        </Text>
                                    </View>
                                    
                                    <View style={S.activeTemplateActions}>
                                        <TouchableOpacity
                                            onPress={() => {
                                                setTemplateSearch("");
                                            ...
                                            activeOpacity={0.7}
                                            style={S.activeTemplateActionBtn}>
                                            <Ionicons name="swap-horizontal" size={13} color={T.gold} />
                                        </TouchableOpacity>
                                        
                                        <TouchableOpacity
                                            onPress={() => setTemplateId(null)}
                                            activeOpacity={0.7}
                                            style={[S.activeTemplateActionBtn, S.activeTemplateClearBtn]}>
                                            <Ionicons name="close" size={14} color={T.inkSoft} />
                                        </TouchableOpacity>
                                    </View>
                            ...
                            <TouchableOpacity
                                ...
                                <LinearGradient
                                    ...
                                    <Text style={S.templatePickText} numberOfLines={1}>
                                        Use a template
                                    </Text>
                                    <Ionicons
                        ...
                        <View style={S.card}>
                            {/* Bulk Email Switch */}
                            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: T.line }}>
                                <View style={{ flex: 1, paddingRight: 10 }}>
                                    <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                                        <Ionicons name="people-outline" size={16} color={isBulk ? T.gold : T.inkMid} />
                                        <Text style={{ fontSize: 14, fontWeight: "600", color: T.ink }}>Bulk Email Mode</Text>
                                    </View>
                                    <Text style={{ fontSize: 11, color: T.inkSoft, marginTop: 2 }}>Send this message to all lead email addresses in your enquiries</Text>
                                </View>
                                <Switch
                                    value={isBulk}
                                    onValueChange={setIsBulk}
                                    trackColor={{ false: T.line, true: T.goldSoft }}
                                    thumbColor={isBulk ? T.gold : "#FFFFFF"}
                                ...
                                <View style={{ marginBottom: 20 }}>
                                    <View style={S.labelRow}>
                                        <Ionicons
                                            ...
                                            style={{ marginRight: 5 }}
                                        />
                                        <Text style={[S.label, { color: T.gold }]}>TO (BULK RECIPIENTS)</Text>
                                    </View>
                                    <View style={[S.input, { justifyContent: "center", backgroundColor: T.surface2, borderColor: T.goldBorder }]}>
                                        <Text style={{ color: T.gold, fontWeight: "600", fontSize: 13 }}>
                                            All Registered Enquiries (Active List)
                                        </Text>
                                    </View>
                                </View>
                            ) : (
            ...
            {tab === "sent" || tab === "logs" ? (
                <View
                    style={{ flex: 1, paddingHorizontal: 18, paddingTop: 20 }}>
                    <FlatList
                        data={tab === "sent" ? groupedLogs : logs}
                        keyExtractor={(item, index) =>
                            String(
                                item?._id
                        ...
                        renderItem={tab === "sent" ? renderGroupedLogRow : renderLogRow}
                        onEndReached={() => {
            ...
            {/* ── RECIPIENT GROUP THREAD HISTORY MODAL ───────────────────────── */}
            <Modal statusBarTranslucent
                ...
                <TouchableOpacity
                    ...
                    <TouchableOpacity
                        activeOpacity={1}
                        onPress={() => {}}
                        style={[S.sheet, { maxHeight: "88%", borderTopLeftRadius: 30, borderTopRightRadius: 30 }]}>
                        <View style={S.sheetHandle} />
                        
                        <View style={S.sheetHeaderRow}>
                            <View style={{ flex: 1, paddingRight: 8 }}>
                                <Text style={S.sheetTitle}>Recipient Thread</Text>
                                <Text style={{ fontSize: 13, color: T.gold, fontWeight: "700", marginTop: 2 }}>
                                    {selectedGroup?.to}
                                </Text>
                            </View>
                            <View
                                ...
                                <Text
                                    ...
                                    {(selectedGroup?.logs || []).length} {(selectedGroup?.logs || []).length === 1 ? "Email" : "Emails"}
                                </Text>
                            </View>
                        </View>
                            ...
                            {(selectedGroup?.logs || []).map((log, idx) => {
                                ...
                                return (
                                    <View
                                        key={log._id || idx}
                                        style={{
                                            backgroundColor: T.surfaceElevation || "#FFFFFF",
                                            borderRadius: 16,
                                            borderWidth: 1,
                                            borderColor: isExpanded ? T.goldBorder : T.line,
                                            padding: 16,
                                            marginBottom: 12,
                                            shadowColor: T.ink,
                                            shadowOpacity: isExpanded ? 0.08 : 0.02,
                                            shadowRadius: 8,
                                        ...
                                        <TouchableOpacity
                                            ...
                                            <View style={{ flex: 1, paddingRight: 8 }}>
                                                <Text style={{ fontSize: 14, fontWeight: "700", color: T.ink, marginBottom: 4 }} numberOfLines={1}>
                                                ...
                                                <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                                                    <View
                                                        style={[
                                                            S.badge,
                                                            {
                                                                backgroundColor: statusBg(log.status),
                                                                borderColor: statusColor(log.status) + "22",
                                                                paddingVertical: 2,
                                                                paddingHorizontal: 6,
                                                            },
                                                        ]}>
                                                        <Text
                                                            style={[
                                                                S.badgeText,
                                                                { color: statusColor(log.status), fontSize: 10, fontWeight: "700" },
                                                            ]}>
                                                            {log.status}
                                                        </Text>
                                                    </View>
                                        ...
                                        {isExpanded && (
                                            <View style={{ borderTopWidth: 1, borderColor: T.line, marginTop: 12, paddingTop: 12 }}>
                                                {/* Message Body */}
                                                <Text style={{ fontSize: 11, fontWeight: "600", color: T.inkSoft, textTransform: "uppercase", marginBottom: 6 }}>
                                                    Message Content
                                                </Text>
                                                <View style={{ backgroundColor: T.background, padding: 12, borderRadius: 8, marginBottom: 12, borderWidth: 1, borderColor: T.line }}>
                                                    <Text style={{ fontSize: 13, color: T.ink, lineHeight: 18 }}>
                                                        {log.body || "(No message body content)"}
                                                    </Text>
                                                </View>

                                                {/* Stats & Counters */}
                                                {(log.openCount > 0 || log.clickCount > 0) && (
                                                    <View style={{ flexDirection: "row", gap: 12, marginBottom: 12, backgroundColor: T.goldSoft, padding: 8, borderRadius: 8 }}>
                                                        {log.openCount > 0 && (
                                                            <Text style={{ fontSize: 12, color: T.gold, fontWeight: "600" }}>
                                                                👁️ Opened: {log.openCount}×
                                                            </Text>
                                                        )}
                                                        {log.clickCount > 0 && (
                                                            <Text style={{ fontSize: 12, color: T.gold, fontWeight: "600" }}>
                                                ...
                                                <View style={{ gap: 4 }}>
                                                    <Text style={{ fontSize: 12, color: T.inkSoft }}>
                                                        <Text style={{ fontWeight: "600" }}>Message ID:</Text> {log.messageId || "-"}
                                                    </Text>
                                                    {log.smtpResponse ? (
                                                        <Text style={{ fontSize: 12, color: T.inkSoft }}>
                                                            <Text style={{ fontWeight: "600" }}>SMTP Response:</Text> {log.smtpResponse}
                                                        </Text>
                                                    ) : null}
                                                    {log.error ? (
                                                        <Text style={{ fontSize: 12, color: T.bad }}>
                                                            <Text style={{ fontWeight: "600" }}>Error Trace:</Text> {log.error}
                                                        </Text>
                                                    ) : null}
                                                </View>
                                            </View>
                        ...
                        <TouchableOpacity
                            ...
                            style={S.sheetClose}>
                            <Text style={S.sheetCloseText}>Close Thread</Text>
                        </TouchableOpacity>
                    </TouchableOpacity>
                </TouchableOpacity>
            </Modal>
                        ...
                        mostUsedTemplates.length === 0 ? (
                            ...
                            <TouchableOpacity
                                ...
                                <Text style={[S.emptyHint, { textAlign: "center" }]}>
                                    Tap <Text style={{ color: T.gold, fontWeight: "800", textDecorationLine: "underline" }}>All</Text> to browse all templates
                                </Text>
                            </TouchableOpacity>
                        )}
            ...
            {bulkProgress ? (
                <Modal statusBarTranslucent visible={true} transparent animationType="fade">
                    <View style={{
                        flex: 1,
                        ...
                        <View style={{
                            ...
                            <Ionicons
                                name={bulkProgress.done ? "checkmark-circle-outline" : "paper-plane-outline"}
                                size={50}
                                color={bulkProgress.done ? T.ok : T.gold}
                                style={{ marginBottom: 16 }}
                            />
                            <Text style={{ fontSize: 18, fontWeight: "700", color: T.ink, marginBottom: 8 }}>
                                {bulkProgress.done ? "Bulk Send Complete" : "Sending Emails"}
                            </Text>
                            <Text style={{ fontSize: 13, color: T.inkSoft, textAlign: "center", marginBottom: 20, paddingHorizontal: 20 }}>
                                {bulkProgress.done 
                                    ? `Successfully processed lead list bulk delivery.`
                                    : `Processing bulk delivery queue...`}
                            </Text>

                            <View style={{ width: "100%", paddingHorizontal: 24, marginBottom: 20 }}>
                                <View style={{ height: 6, width: "100%", backgroundColor: T.line, borderRadius: 3, overflow: "hidden" }}>
                                    <View style={{ height: "100%", width: `${bulkProgress.total > 0 ? (bulkProgress.current / bulkProgress.total) * 100 : 0}%`, backgroundColor: T.gold }} />
                                </View>
                                <Text style={{ fontSize: 12, fontWeight: "600", color: T.inkSoft, marginTop: 8, textAlign: "right" }}>
                                    {bulkProgress.current} / {bulkProgress.total}
                                </Text>
                            </View>

                            <View style={{ flexDirection: "row", justifyContent: "space-around", width: "100%", paddingHorizontal: 20, marginBottom: 24 }}>
                                <View style={{ alignItems: "center" }}>
                                    <Text style={{ fontSize: 16, fontWeight: "700", color: T.ink }}>{bulkProgress.success}</Text>
                                    <Text style={{ fontSize: 11, color: T.ok, fontWeight: "600" }}>SENT</Text>
                                </View>
                                <View style={{ alignItems: "center" }}>
                                    <Text style={{ fontSize: 16, fontWeight: "700", color: T.ink }}>{bulkProgress.failed}</Text>
                                    <Text style={{ fontSize: 11, color: T.bad, fontWeight: "600" }}>FAILED</Text>
                                </View>
                            </View>

                            {bulkProgress.done && (
                                <TouchableOpacity 
                                    onPress={() => setBulkProgress(null)}
                                    style={{ backgroundColor: T.gold, paddingVertical: 12, paddingHorizontal: 30, borderRadius: 10 }}>
                                    <Text style={{ color: "#FFFFFF", fontWeight: "600", fontSize: 14 }}>Dismiss</Text>
                                </TouchableOpacity>
                            )}
                        </View>
                    </View>
