...
import { ActivityIndicator,
    ...
    TextInput,
    TouchableOpacity,
    View, StatusBar, KeyboardAvoidingView, Platform } from "react-native";
...
const ProgressRow = ({ label, actual, target, color, formatter }) => {
    ...
    return (
        <View style={{ gap: 6 }}>
            ...
            <Text
                style={{ fontSize: 11, color: C.textSoft, fontWeight: "700" }}>
                {p}% achieved
            </Text>
        </View>
    );
...
export default function TargetsScreen({ navigation }) {
    ...
    const load = async ({ showSpinner } = { showSpinner: true }) => {
        try {
            ...
            // Only update form fields if the form is not open (prevents reset while editing)
            if (!formOpen) {
