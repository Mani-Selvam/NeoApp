...
const getAssignedUserLabel = (assignedTo, currentUser = null) => {
    if (!assignedTo) return "-";
    if (typeof assignedTo === "string") {
        if (currentUser && (assignedTo === String(currentUser.id || "") || assignedTo === String(currentUser._id || ""))) {
            return currentUser.name || "Me";
        }
        if (assignedTo.length === 24 && /^[0-9a-fA-F]{24}$/.test(assignedTo)) {
            return "Staff";
        }
        return assignedTo;
    }
                                    ...
                                    {[
                                        ...
                                        },
                                        {
                                            ...
                                            value: getAssignedUserLabel(
                                                enquiry.assignedTo || enquiry.userId,
                                                user
                                            ),
...
export default function EnquiryListScreen({ navigation, route }) {
    ...
    const handleCall = useCallback(async (enquiry) => {
        ...
        try {
            ...
            if (
                immediateCall?.immediatePhoneCall &&
                !playStoreSafeMode // Using the value from the top of the file
            ) {
                immediateCall.immediatePhoneCall(raw);
                return;
            }
        ...
        <SafeAreaView style={S.root} edges={["top"]}>
            ...
            <Animated.View
                ...
                <TouchableOpacity
                    ...
                    activeOpacity={0.85}>
                    <LinearGradient colors={GRAD.primary} style={{ width: 56, height: 56, borderRadius: 28, alignItems: "center", justifyContent: "center", shadowColor: "#7B61FF", shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 12, elevation: 8 }}>
            ...
            {/* ── URL Scraping Modal (Centered Popup) ── */}
            <Modal statusBarTranslucent
