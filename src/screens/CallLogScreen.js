...
const FILTER_TYPES = ["All", "Missed", "Not Attended", "Incoming", "Outgoing"];
const TIME_PERIODS = [
    { key: "All", label: "All" },
        ...
        <TouchableOpacity
            ...
            <View
                style={[
                    CS.ctrlCircle,
                    active && {
                        backgroundColor: color || "rgba(255,255,255,0.22)",
                    },
                ]}>
                <Ionicons
                    name={icon}
            ...
            <Text style={[CS.ctrlLabel, active && { color: "#fff" }]}>
                {label}
            </Text>
        </TouchableOpacity>
    );
...
export default function CallLogScreen({ navigation, route, embedded = false }) {
    ...
    const buildDefaultNote = (callType, durationSeconds) => {
        ...
        if (callType === "Incoming")
            return safeDur > 0
                ? `Incoming call • ${safeDur}s`
                : "Incoming call";
        if (callType === "Outgoing")
            return safeDur > 0
                ? `Outgoing call • ${safeDur}s`
                : "Outgoing call";
    ...
    const autoSavePendingCall = useCallback(
        async (pc, payload = {}) => {
            ...
            try {
                ...
                const callTime =
                    device?.callTime || payload?.callTime || new Date();
    ...
    const syncControl = useCallback(
        async (action, value, extra = {}) => {
            ...
            if (action === "dtmf")
                patch.keypadDigits =
                    `${cur.keypadDigits || ""}${String(value || "")}`.slice(
                        -32,
                    );
            updateCall(patch);
            try {
    ...
    const renderCallRow = ({ item }) => {
        ...
        const isNotAttended = item?.callType === "Not Attended";
        const isUnanswered = isMissed || isNotAttended;
        // Only show duration for answered calls (Incoming/Outgoing with duration > 0)
            ...
            <Swipeable
                ...
                <TouchableOpacity
                    ...
                    <View style={{ position: "relative" }}>
                        ...
                        <View
                            style={[R.typeDot, { backgroundColor: cfg.color }]}
                        />
                    </View>
            ...
            {!DEVICE_SYNC_ENABLED && Platform.OS === "android" ? (
                <View
                    ...
                    <Text
                        ...
                        Set `EXPO_PUBLIC_PLAY_STORE_SAFE_MODE=false` and rebuild
                        the Android app to enable Incoming/Outgoing/Missed sync
                        like the Phone app.
                    </Text>
                </View>
            ) : null}
    ...
    return (
        <SafeAreaView
            ...
            {/* ── Header ── */}
            {!embedded ? (
                <View style={[R.header, { paddingHorizontal: sc.hPad }]}>
                    <View style={{ flex: 1 }}>
                        <Text
                            style={{
                                fontSize: 11,
                    ...
                    <CallOverlay
                        ...
                        onToggleSpeaker={async (v) => {
                            ...
                            syncControl("speaker", v, {
                                nativeApplied: n,
                                nativeSupported:
                                    getInCallControlSupport().speaker,
                            });
                        }}
