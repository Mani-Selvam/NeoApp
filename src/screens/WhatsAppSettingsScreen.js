...
const mergeConfigToForm = (config = {}) => ({
    ...
    templateButtonIndex: String(typeof config.templateButtonIndex === "number" ? config.templateButtonIndex : 0),
    enquiryAlertTemplateName: config.enquiryAlertTemplateName || "",
        ...
        {STEPS.map((step, idx) => {
            ...
            return (
                <View key={step.id} style={SB.item}>
                    ...
                    {idx < STEPS.length - 1 && (
                        <View
                            style={[SB.connector, done && SB.connectorDone]}
                        />
                    )}
                </View>
            );
    ...
    return (
        <TouchableOpacity
            ...
            <Text
                style={[
                    PC.sub,
                    isActive && { color: colors.text, opacity: 0.7 },
                ]}>
                {meta.sub}
            </Text>
            {isActive && (
...
export default function WhatsAppSettingsScreen({ navigation }) {
    ...
    const handleOtpDigit = (idx, val) => {
        ...
        // If pasting multiple digits (e.g., full OTP code)
        if (numericOnly.length > 1) {
            // Distribute pasted digits across all boxes
            ...
            const lastFilledIdx = Math.min(numericOnly.length - 1, 5);
            setTimeout(() => otpRefs[lastFilledIdx].current?.focus(), 100);
            return;
        }
        
        // Single digit input - limit to 1 character
        const singleDigit = numericOnly.slice(0, 1);
        const digits = [...otpDigits];
        digits[idx] = singleDigit;
        setOtpDigits(digits);
        setOtpCode(digits.join(""));
        
        // Auto-focus to next box if digit entered
        if (singleDigit && idx < 5) {
            otpRefs[idx + 1].current?.focus();
        }
        // Auto-focus to previous box if digit deleted
        if (!singleDigit && idx > 0) {
            otpRefs[idx - 1].current?.focus();
        }
    };
    ...
    const handleCopyTemplate = async () => {
        const text = `Hello Dear {{1}} Customer,\n\nThank you for your enquiry regarding {{2}} service.\n\nWe have received your request and our team will contact you shortly.\n\nIf you have any additional details to share, please reply to this message.\n\nRegards from {{3}} Team`;
        await Clipboard.setStringAsync(text);
        Alert.alert(
            "⚠️ Copied Successfully",
            "Template text has been copied to your clipboard.\n\nIMPORTANT: DO NOT change the variable numbers {{1}}, {{2}}, and {{3}} when submitting this template to WhatsApp Manager. Only modify the surrounding text if needed."
        );
    };
    ...
    const validateForm = () => {
        ...
        if (form.provider === PROVIDERS.META)
            return (
                form.metaWhatsappToken.trim() && form.metaPhoneNumberId.trim()
            );
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                templateButtonIndex: Number(form.templateButtonIndex) || 0,
                enquiryAlertTemplateName: form.enquiryAlertTemplateName.trim(),
            ...
            if (resp.data?.ok) {
                ...
                setForm((prev) =>
                    mergeConfigToForm({ ...resp.data.config, ...prev }),
                );
                await loadConfig();
                Alert.alert(
                    "Saved!",
            ...
            <View style={S.header}>
                ...
                <View
                    style={[
                        S.headerStatusPill,
                        isConnected
                            ? S.headerStatusPillOn
                            : S.headerStatusPillOff,
                    ]}>
                    <View
                        style={[
                            S.headerStatusDot,
                            ...
                            <MotiView
                                    ...
                                    {isConnected && (
                                        <View style={S.providerInfoRow}>
                                            <View style={S.providerInfoItem}>
                                                <Text
                                                    style={S.providerInfoLabel}>
                                                    Provider
                                                </Text>
                                                <Text
                                                    style={S.providerInfoValue}>
                                                    {waConfig.provider}
                                                </Text>
                                            </View>
                                            <View
                                                style={S.providerInfoDivider}
                                            />
                                            <View style={S.providerInfoItem}>
                                                <Text
                                                    style={S.providerInfoLabel}>
                                ...
                                <View style={S.secNote}>
                                    ...
                                    <Text style={S.secNoteText}>
                                        Your credentials are end-to-end
                                ...
                                <View style={S.templateBox}>
                                    <View style={S.templateBoxHeader}>
                                        <Text style={S.templateBoxTitle}>Enquiry Template Content</Text>
                                        <TouchableOpacity onPress={handleCopyTemplate} style={S.copyBtn}>
                                            <Ionicons name="copy-outline" size={16} color={T.brand} />
                                            <Text style={S.copyBtnText}>Copy Text</Text>
                                        </TouchableOpacity>
                                    </View>
                                    <Text style={S.templateBoxContent}>
                                        {`Hello Dear {{1}} Customer,\n\nThank you for your enquiry regarding {{2}} service.\n\nWe have received your request and our team will contact you shortly.\n\nIf you have any additional details to share, please reply to this message.\n\nRegards from {{3}} Team`}
                                    </Text>
                                    <View style={S.templateBoxFooter}>
                                        <Text style={S.templateBoxNote}>Variables: 1 → Name, 2 → Product, 3 → Company Name</Text>
                                    </View>
                                </View>
                            </MotiView>
                        )}
                                        ...
                                        {form.provider === PROVIDERS.NEO && (
                                            <>
                                                ...
                                                <FloatingInput
                                                    ...
                                                    value={form.templateButtonIndex}
                                                    onChange={(v) =>
                                                        updateField(
                                                            "templateButtonIndex",
                                                            v,
                                                ...
                                                />
                                                <FloatingInput
                                                    label="Enquiry Alert Template Name (Required)"
                                                    value={form.enquiryAlertTemplateName}
                                                    onChange={(v) =>
                                                        updateField(
                                                            "enquiryAlertTemplateName",
                                                            v,
                                                    ...
                                                    placeholder="e.g. enquiry_confirmation"
                                                />
                                            </>
                                        )}
                                    </View>
