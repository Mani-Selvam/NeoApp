...
const getFeatures = (plan) => {
    ...
    const baseRows = [
        {
            icon: "people-outline",
            label: `${plan?.maxStaff || 0} staff members`,
        },
        {
            icon: "shield-checkmark-outline",
            label: `${plan?.maxAdmins || 0} admin account${(plan?.maxAdmins || 0) !== 1 ? "s" : ""}`,
        },
        ...(plan?.trialDays > 0
            ? [
                  {
            ...
            <View
                    ...
                    <LinearGradient
                        ...
                        <View style={{ alignItems: "flex-end", gap: 10 }}>
                            <View
                                style={[
                                    PCS.radio,
                                    selected && PCS.radioSelected,
                                ]}>
                                {selected && <View style={PCS.radioDot} />}
                            </View>
                            <View style={{ alignItems: "flex-end" }}>
                                <Text
                                    style={[PCS.hPrice, { fontSize: sc.f.xl }]}>
                                    {displayPrice}
                                </Text>
                                <Text style={[PCS.hPer, { fontSize: sc.f.xs }]}>
                                    per month
                                </Text>
                            </View>
                        </View>
                ...
                <TouchableOpacity
                    ...
                    <View style={PCS.statsRow}>
                        {[
                            ...
                            {
                                num: plan?.trialDays > 0 ? plan.trialDays : "—",
                                label: "Trial days",
                            },
                        ].map((s, i) => (
                ...
                <TouchableOpacity
                    ...
                    <Text
                        style={[
                            PCS.expandLabel,
                            { fontSize: sc.f.sm, color: col.dot },
                        ]}>
                        {open
                            ? "Hide features"
                            : `View all ${feats.length} features`}
                    </Text>
                    <View
                        style={[
                            PCS.expandChevron,
    ...
    const loadPlans = useCallback(async () => {
        try {
            ...
            const mapped = (res?.plans || []).map((p) => ({
                ...
                extraAdminPrice: Number(p.extraAdminPrice || 0),
                extraStaffPrice: Number(p.extraStaffPrice || 0),
            ...
            setSelectedId((prev) => {
                ...
                if (
                    prev &&
                    mapped.some((plan) => String(plan.id) === String(prev))
                ) {
                    return prev;
                }
            ...
            console.error(
                "[PricingScreen] Failed to load plans:",
                err?.message,
            );
            setError(
                err?.message ||
                    "Failed to load plans. Please check your connection.",
            );
            setPlans([]);
        } finally {
            setLoading(false);
        ...
        <SafeAreaView
            ...
            <View style={[S.nav, { paddingHorizontal: sc.hPad }]}>
                ...
                <View style={{ flex: 1, alignItems: "center" }}>
                    <Text style={[S.navTitle, { fontSize: sc.f.md }]}>
                        Choose a Plan
                    </Text>
                </View>
                {/* Currency toggle — ₹ / $ */}
            ...
            <ScrollView
                ...
                <View style={[S.hero, { borderRadius: sc.r + 4 }]}>
                    ...
                    <View
                        ...
                        <Text style={[S.heroEye, { fontSize: sc.f.xs }]}>
                            UPGRADE YOUR CRM
                        </Text>
                    </View>
                    <Text style={[S.heroEye, { fontSize: sc.f.xl }]}>
                        Grow without limits
                    </Text>
                </View>
                ...
                <View style={S.plansHeader}>
                    <Text style={[S.plansTitle, { fontSize: sc.f.md }]}>
                        Available Plans
                    </Text>
                    {!loading && (
                ...
                {loading ? (
                    ...
                    <View
                        style={[
                            S.emptyBox,
                            { borderRadius: sc.r, backgroundColor: "#FEE2E2" },
                        ]}>
                        <Ionicons
                    ...
                    <View>
                        {plans.map((plan) => (
                            <PlanCard
                                ...
                                plan={plan}
                                selected={
                                    String(selectedId) === String(plan.id)
                                }
                                isCurrent={
                ...
                <View style={S.trust}>
                    {[
                        ...
                        <View key={t.text} style={S.trustItem}>
                            ...
                            <Text style={[S.trustText, { fontSize: sc.f.xs }]}>
                                {t.text}
                            </Text>
                        </View>
                    ))}
                ...
                {selected && (
                    <View style={[S.summaryRow, { borderRadius: sc.r }]}>
                        <View
                            style={[
                                S.sumDot,
                                { backgroundColor: selTier.dot + "22" },
                            ]}>
                            <View
                                style={[
                                    S.sumDotInner,
                                    { backgroundColor: selTier.dot },
                                ]}
                            />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={[S.sumLabel, { fontSize: sc.f.xs }]}>
                                Selected
                            </Text>
                            <Text
                                style={[S.sumName, { fontSize: sc.f.md }]}
                                numberOfLines={1}>
                                {selected.name}
                            </Text>
                            {isCurrentSelection ? (
                        ...
                        <Text
                            style={[
                                S.sumPrice,
                                { fontSize: sc.f.lg, color: selTier.dot },
                            ]}>
                            {getTier(selected) === "enterprise"
                                ? "Custom"
                                : getPrice(selected)}
                        </Text>
                    </View>
                            ...
                            {activatingFree
                                ...
                                : isCurrentSelection
                                  ...
                                  : selected &&
                                    ...
                                    ? "Contact Sales"
                                    : selected &&
                                        Number(selected?.basePriceUsd || 0) <= 0
                                      ? "Activate Free Plan"
                                      : "Continue"}
                        </Text>
...
const S = StyleSheet.create({
    ...
    countPill: {
        backgroundColor: T.pro.soft,
        paddingHorizontal: 10,
        paddingVertical: 4,
        borderRadius: 99,
        borderWidth: 1,
        borderColor: T.pro.a + "30",
    },
    emptyBox: {
        flexDirection: "row",
        alignItems: "center",
