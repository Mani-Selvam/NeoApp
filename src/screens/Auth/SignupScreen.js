...
const SignupScreen = ({ navigation }) => {
    ...
    const handleSaveCollectedMobile = async () => {
        ...
        if (!val) {
            showInline("Please enter your mobile number", "error");
            return;
        }
        if (val.length !== 10) {
            showInline("Mobile number must be 10 digits", "error");
            return;
        }
        ...
        try {
            ...
            if (res.data.success) {
                ...
                showInline("Failed to save mobile number", "error");
            }
        } catch (err) {
            showInline(err.response?.data?.message || "Failed to update mobile number", "error");
        } finally {
