...
export default function CommunicationScreen({ navigation }) {
  ...
  const contactList = useMemo(() => {
    ...
    return team
      ...
      .map((m) => ({ member: m, thread: threadMap.get(String(m._id)) || null }))
      .filter(
        (c) =>
          !searchQuery.trim() ||
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert("Error", e?.response?.data?.error || "Failed to load tasks");
    } finally {
  ...
  // ── Call events ────────────────────────────────────────────────────────────
  useEffect(() => {
    const sub = DeviceEventEmitter.addListener("CALL_ENDED", async (event) => {
      ...
      const eventAt = new Date(event?.callTime || Date.now()).toISOString();
      const key = [
        String(member._id),
        status,
  ...
  const handleInitiateCall = useCallback(async (member) => {
    ...
    if (!rawPhone) {
      Alert.alert("No phone number", "This team member has no phone number.");
      return;
    }
    try {
      if (Platform.OS === "android") {
        ...
        if (
          ...
          Alert.alert("Permission denied", "Phone permission is required.");
          return;
        }
      }
      let started = false;
    ...
    try {
      ...
      Alert.alert(
        ...
        error?.response?.data?.error ||
          getUserFacingError(error, "Unable to send voice recording."),
      );
    } finally {
      resetRecordingState();
    }
    ...
    setTaskAttachment(
      task?.attachmentUrl
        ? {
            ...
            type: task.attachmentMimeType || inferAttachmentType(task),
  ...
  const toggleAudioPlayback = useCallback(
    async (item) => {
      ...
      const audioUri = item?.attachmentUrl ? getImageUrl(item.attachmentUrl) : "";
      ...
      try {
        ...
        Alert.alert(
          "Playback failed",
          getUserFacingError(error, "Unable to play this voice message."),
        );
      }
    },
  ...
  if (loading) {
    return (
      <SafeAreaView style={S.screen} edges={["left", "right"]}>
        ...
        <View style={{ paddingHorizontal: 18, paddingTop: insets.top + 18 }}>
          <SkeletonPulse>
    ...
    const renderMessageItem = ({ item }) => {
          ...
          <View style={S.callEventWrap}>
            ...
            >
              <View
                style={[
                  S.callEventIcon,
                  { backgroundColor: "#fff", borderColor: meta.border },
                ]}
              >
                <Ionicons name={meta.icon} size={15} color={meta.tint} />
              </View>
              <View style={S.callEventTextWrap}>
                <Text style={[S.callEventTitle, { color: meta.tint }]}>
                  {meta.title}
                </Text>
                <Text style={S.callEventMeta}>
                  {formatClock(item?.callTime || item?.createdAt)}
      ...
      const isAudioMessage =
        ...
        String(item?.attachmentMimeType || "")
          .toLowerCase()
          .startsWith("audio/");
      const isThisAudioPlaying = playingAudioId === String(item?._id || "") && isPlayingAudio;
      return (
        <View style={[S.msgRow, isMine ? S.msgRowOut : S.msgRowIn]}>
          {!isMine && (
            <InitialsAvatar name={selectedMember?.name || "?"} size={30} />
          )}
            <View style={[S.msgBubble, isMine ? S.msgBubbleOut : S.msgBubbleIn]}>
              {item.attachmentUrl ? (
                item.messageType === "image" ? (
                  ...
                  <TouchableOpacity
                    style={[S.audioBubble, isMine && S.audioBubbleOut]}
                    activeOpacity={0.88}
                    onPress={() => toggleAudioPlayback(item)}
                  >
                    <View style={[S.audioPlayBtn, isMine && S.audioPlayBtnOut]}>
                      <Ionicons
                    ...
                    <View style={S.audioInfo}>
                      <Text style={S.audioTitle}>Voice message</Text>
                      <Text style={S.audioMeta}>
                        {item?.callDuration > 0
                          ? formatCallDuration(item.callDuration)
                          : item?.attachmentName || "Tap to play"}
                      </Text>
                    </View>
                  </TouchableOpacity>
              ) : (
                  <View style={S.docRow}>
                    <View style={[S.docIconWrap, isMine && S.docIconWrapOut]}>
                      <Ionicons
                        name={
                          String(item?.attachmentMimeType || "")
                            .toLowerCase()
      ...
      >
        ...
        <View style={[S.chatHeader, { paddingTop: insets.top + 6 }]}>
          ...
          <View style={S.chatHeaderInfo}>
            <Text style={S.chatHeaderName}>{selectedMember.name}</Text>
          ...
          <View style={S.chatHeaderActions}>
            {isAdminUser && (
              <TouchableOpacity
                ...
                onPress={() => openCreateTaskModal(String(selectedMember._id))}
              >
                <Ionicons
                  name="create-outline"
                  size={22}
            ...
            >
              <Ionicons name="call-outline" size={20} color={T.ink} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        */}
        ...
        >
            ...
            {messageLoading ? (
              ...
              <FlatList
                ...
                contentContainerStyle={[
                  S.msgList,
                  { paddingBottom: messageAttachment ? 18 : 10 },
                ]}
            ...
            <View style={S.voiceDraftBar}>
              <View style={S.voiceDraftInfo}>
                <View style={S.voiceDraftIcon}>
                  <Ionicons name="mic" size={16} color={T.accentDark} />
                </View>
                <View style={S.voiceDraftTextWrap}>
                  <Text style={S.voiceDraftTitle}>Voice note ready</Text>
                  <Text style={S.voiceDraftMeta}>
                    {formatRecordingDuration(audioDraft.durationMs)}
                  </Text>
                </View>
              </View>
              <View style={S.voiceDraftActions}>
                <TouchableOpacity
                  style={[S.voiceDraftBtn, S.voiceDraftDeleteBtn]}
                ...
                <TouchableOpacity
                  style={[S.voiceDraftBtn, S.voiceDraftSendBtn]}
          ...
          */}
          <View
            style={[S.composer, { paddingBottom: Math.max(insets.bottom , 8) }]}
          >
            <View style={S.composerInputWrap}>
              ...
              >
                <Ionicons name="happy-outline" size={22} color={T.mute} />
              </TouchableOpacity>
              <TextInput
              ...
              />
              <TouchableOpacity
                onPress={() => pickDocument(setMessageAttachment)}
                style={S.composerIconBtn}
              >
                <Ionicons name="attach-outline" size={22} color={T.mute} />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={pickMessageImage}
                style={S.composerIconBtn}
              >
                <Ionicons name="camera-outline" size={22} color={T.mute} />
              </TouchableOpacity>
            </View>
            <TouchableOpacity
              style={[
                S.composerSend,
                (messageText.trim() || messageAttachment || isRecordingAudio) &&
                  S.composerSendActive,
                isRecordingAudio && S.composerSendRecording,
              ]}
              onPress={handleComposerPrimaryPress}
              disabled={sendingMessage || Boolean(audioDraft?.uri)}
            >
              {sendingMessage ? (
                <ActivityIndicator size="small" color="#fff" />
              ) : (
      ...
      >
        ...
        <View style={S.contactInfo}>
          <View style={S.contactTopRow}>
            ...
            {lastTime ? (
              <Text style={[S.contactTime, unread > 0 && S.contactTimeUnread]}>
                {lastTime}
              </Text>
            ) : null}
          </View>
          <View style={S.contactBottomRow}>
            <Text
              style={[S.contactPreview, unread > 0 && S.contactPreviewUnread]}
              numberOfLines={1}
            >
              {lastMsg || roleLabel}
            </Text>
      ...
      >
        ...
        <View style={S.taskInner}>
          <View style={S.taskTopRowCard}>
            <Text
              style={[S.taskTitle, isCompleted && S.taskTitleDone]}
              numberOfLines={2}
            >
              {item.title}
            </Text>
            ...
            >
              <Text style={[S.statusTagText, { color: st.text }]}>
                {item.status}
              </Text>
            </View>
          </View>
          <View style={S.taskMetaRow}>
            <Text style={S.taskMetaItem}>{item.taskType}</Text>
            <View style={S.taskMetaDot} />
            <Text style={S.taskMetaItem}>{item.priority}</Text>
            <View style={S.taskMetaDot} />
            <Text style={S.taskMetaItem}>{formatDate(item.dueDate)}</Text>
          </View>
          {item.description ? (
          ...
          <View style={S.taskPeople}>
            <View style={S.taskPersonRow}>
              <Text style={S.taskPersonLabel}>Assigned</Text>
              <Text style={S.taskPersonName}>
                {item.assignedTo?.name || "Unassigned"}
              </Text>
            </View>
            <View style={S.taskPersonRow}>
              <Text style={S.taskPersonLabel}>By</Text>
              <Text style={S.taskPersonName}>
                {item.createdBy?.name || "—"}
              </Text>
            </View>
            ...
            <View style={S.taskAttachmentBlock}>
              ...
              <AttachmentPill
                attachment={{
                  ...
                  type: item.attachmentMimeType || inferAttachmentType(item),
                }}
              />
            </View>
          )}
          <View style={S.taskBtnRow}>
            {canEditTask && (
              ...
              >
                ...
                <Text style={S.taskActionTextGhost}>Edit</Text>
              </TouchableOpacity>
            )}
            {canUpdateStatus && (
              <TouchableOpacity
                style={[
                  S.taskAction,
                  isCompleted ? S.taskActionGhost : S.taskActionPrimary,
                ]}
                onPress={() =>
                  setTaskStatus(item._id, isCompleted ? "Pending" : "Completed")
                }
              >
                <Ionicons
                  name={isCompleted ? "refresh-outline" : "checkmark"}
                  size={13}
                  color={isCompleted ? T.mid : "#fff"}
                />
                <Text
                  style={[
                    S.taskActionText,
                    isCompleted && S.taskActionTextGhost,
                  ]}
                >
            ...
            {canUpdateStatus &&
              ...
              !isCompleted && (
                <TouchableOpacity
                  style={[S.taskAction, S.taskActionGhost]}
                  onPress={() => setTaskStatus(item._id, "In Progress")}
                >
                  <Text style={S.taskActionTextGhost}>Start</Text>
                </TouchableOpacity>
              )}
          </View>
        </View>
              ...
              >
                ...
                <View style={S.chipRow}>
                  {["Low", "Medium", "High"].map((opt) => (
                    <TouchableOpacity
                      ...
                      style={[
                        S.fChip,
                        taskForm.priority === opt && S.fChipActive,
                      ]}
                      onPress={() =>
                        setTaskForm((p) => ({ ...p, priority: opt }))
                      }
                    >
                      <Text
                        style={[
                          S.fChipTxt,
                          taskForm.priority === opt && S.fChipTxtActive,
                        ]}
                      >
                        {opt}
                      </Text>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={S.assignRow}>
                    <TouchableOpacity
                      style={[S.fChip, !taskForm.assignedTo && S.fChipActive]}
                      onPress={() =>
                        setTaskForm((p) => ({ ...p, assignedTo: "" }))
                      }
                    >
                      <Text
                        style={[
                          S.fChipTxt,
                          !taskForm.assignedTo && S.fChipTxtActive,
                        ]}
                      >
                        Optional
                      </Text>
                    ...
                    {taskAssigneeOptions.map((member) => (
                      <TouchableOpacity
                        ...
                        style={[
                          S.fChip,
                          String(taskForm.assignedTo) === String(member._id) &&
                            S.fChipActive,
                        ]}
                        onPress={() =>
                          setTaskForm((p) => ({
                            ...p,
                            assignedTo: String(member._id),
                          }))
                        }
                      >
                        <Text
                          style={[
                            ...
                            String(taskForm.assignedTo) ===
                              String(member._id) && S.fChipTxtActive,
                          ]}
                        >
                          {member.name}
                        </Text>
                ...
                >
                  <Ionicons name="attach-outline" size={16} color={T.mid} />
                  <Text style={S.fFilePickerTxt}>Photo or PDF</Text>
                </TouchableOpacity>
                {showTaskAttachmentImage && taskAttachment?.uri ? (
                ...
                />
                <TouchableOpacity
                  style={[S.submitBtn, taskSaving && { opacity: 0.55 }]}
                  onPress={submitTask}
                  disabled={taskSaving}
                >
                  {taskSaving ? (
      ...
      <View style={S.tabBar}>
        {["Chats", "Pending", "Completed"].map((item) => (
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <Text style={[S.tabTxt, tab === item && S.tabTxtActive]}>
              {item}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        <>
          ...
          >
            {groupedContacts.admins.length > 0 && (
              <>
                {renderSectionHeader("ADMINS", groupedContacts.admins.length)}
                <FlatList
            ...
            {contactList.length === 0 && (
              <View style={S.emptyWrap}>
                <Ionicons name="people-outline" size={48} color={T.mute} />
        ...
        >
          <View style={S.taskListHeader}>
            <Text style={S.taskListTitle}>
              {tab === "Completed" ? "Completed Tasks" : "Pending Tasks"}
            </Text>
...
const S = StyleSheet.create({
  ...
  taskMetaItem: { fontSize: 11, fontWeight: "600", color: T.mute },
  taskMetaDot: {
    width: 3,
    height: 3,
    borderRadius: 1.5,
  ...
  taskDesc: { fontSize: 13, lineHeight: 20, color: T.mid, marginBottom: 10 },
  taskPeople: { flexDirection: "row", gap: 16, marginBottom: 10 },
  taskPersonRow: { flexDirection: "row", gap: 6, alignItems: "center" },
  taskPersonLabel: { fontSize: 11, color: T.mute, fontWeight: "600" },
  taskPersonName: { fontSize: 12, color: T.ink, fontWeight: "700" },
  ...
  taskBtnRow: { flexDirection: "row", gap: 8, marginTop: 4 },
  taskAction: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
  ...
  taskActionText: { fontSize: 12, fontWeight: "700", color: "#fff" },
  taskActionTextGhost: { fontSize: 12, fontWeight: "700", color: T.mid },
  taskEmpty: { alignItems: "center", paddingVertical: 60, gap: 12 },
  taskEmptyText: {
    fontSize: 14,
    color: T.mute,
    textAlign: "center",
