...
const getTaskRemarkEntries = (task) => {
    const list = Array.isArray(task?.statusHistory) ? task.statusHistory : [];
    const normalized = list
        .map((row) => ({
            ...
            updatedById: String(row?.updatedBy?._id || row?.updatedBy || ""),
        }))
...
export default function CommunicationScreen({ navigation }) {
    ...
    const [taskRemarkSaving, setTaskRemarkSaving] = useState(false);
    const [taskRemarkText, setTaskRemarkText] = useState("");
    const [taskRemarkTarget, setTaskRemarkTarget] = useState(null);
    ...
    // Auto-refresh communication overview every 15 seconds while active (SILENTLY)
    useSilentRefresh(() => loadOverview({ silent: true }), 15000);
    ...
    const memberTaskCounts = useMemo(() => {
        ...
        for (const t of allTasks) {
            ...
            if (!mapping[assignedId]) {
                mapping[assignedId] = { pending: 0, inProgress: 0, completed: 0 };
            }

            const s = String(t?.status || "").trim();
            if (s === "Completed") {
                mapping[assignedId].completed += 1;
            } else if (s === "In Progress") {
                mapping[assignedId].inProgress += 1;
            } else if (s === "Cancelled") {
                // skip
            } else {
                mapping[assignedId].pending += 1;
            }
        }
        return mapping;
    }, [allTasks]);
    ...
    const handleInitiateCall = useCallback(async (member) => {
        ...
        try {
            if (Platform.OS === "android" && !playStoreSafeMode) {
                const { ensurePhoneCallPermission } = require("../utils/phoneCallPermissions");
                const { granted, status } = await ensurePhoneCallPermission();

                if (granted) {
                    const immediateCall = getImmediateCall();
                    if (immediateCall?.immediatePhoneCall) {
                        try {
                            immediateCall.immediatePhoneCall(rawPhone);
                            return;
                        } catch (directErr) {
                            console.warn("Direct phone call failed", directErr);
                        }
                    }
                    ...
                    if (IntentLauncher?.startActivityAsync) {
                        try {
                            ...
                            console.warn("Intent direct call failed", intentErr);
                        }
                    }

                    Alert.alert(
                        "Direct call unavailable",
                        "Could not start a direct phone call. Please ensure call permission is granted and try again.",
                    );
                    return;
                }

                if (status === "never_ask_again") {
                    await Linking.openURL(`tel:${url.replace("tel:", "")}`);
                    return;
                }

                Alert.alert(
                    "Call permission needed",
                    "Allow Phone permission to start the call directly from the app, or use the dialer.",
                    [
    ...
    const toggleAudioPlayback = useCallback(
        async (item) => {
            if (!Audio?.Sound?.createAsync || !Audio?.setAudioModeAsync) {
                Alert.alert(
                    "Not supported",
                    "Audio playback is not available on web.",
                );
                return;
            }
    ...
    const setTaskStatus = useCallback(
        async (taskId, status, remark = "") => {
            ...
            if (
                ...
                Alert.alert(
                    "Remark Required",
                    "Please add a remark before updating this task status.",
                );
                return false;
            }
            try {
                await updateCommunicationTaskStatus(taskId, status, cleanRemark);
                const keepTaskDetailsOpen =
                    view === "taskDetail" || showTaskDetailModal;
                if (!keepTaskDetailsOpen) {
                    setView("list");
                ...
                // Reopen moves task back to pending bucket; keep detail screen valid.
                if (status === "Pending" && tab === "Completed") {
                    setTab("Pending");
                }
        ...
        [loadTasks, showTaskDetailModal, tab, view],
    );

    const openTaskRemarkModal = useCallback((task, status) => {
    ...
    const canManageTaskRemark = useCallback(
        (row) => {
            const ownerId = String(row?.updatedById || "");
            if (!ownerId) return isAdminUser;
            return isAdminUser || ownerId === selfId;
        },
        [isAdminUser, selfId],
    );

    const openEditTaskRemarkModal = useCallback(
        (task, row) => {
            if (!task?._id || !row?.id) return;
            if (!canManageTaskRemark(row)) return;
            setTaskRemarkTarget({
                taskId: String(task._id),
                title: String(task?.title || "Task"),
                mode: "editRemark",
                remarkId: String(row.id),
            });
            setTaskRemarkText(String(row.remark || ""));
            setShowTaskRemarkModal(true);
        },
        [canManageTaskRemark],
    );

    const deleteTaskRemark = useCallback(
        (task, row) => {
            if (!task?._id || !row?.id) return;
            if (!canManageTaskRemark(row)) return;
            Alert.alert("Delete remark", "Delete this remark?", [
                { text: "Cancel", style: "cancel" },
                {
                    text: "Delete",
                    style: "destructive",
                    onPress: async () => {
                        try {
                            await deleteCommunicationTaskRemark(
                                task._id,
                                row.id,
                            );
                            await loadTasks();
        ...
        [canManageTaskRemark, loadTasks],
    );

    const closeTaskRemarkModal = useCallback(() => {
        if (taskRemarkSaving) return;
        setShowTaskRemarkModal(false);
        setTaskRemarkTarget(null);
        setTaskRemarkText("");
    }, [taskRemarkSaving]);

    const submitTaskRemarkUpdate = useCallback(async () => {
        if (!taskRemarkTarget?.taskId) return;
        setTaskRemarkSaving(true);
        try {
            let ok = false;
            if (taskRemarkTarget?.mode === "editRemark") {
                const nextRemark = String(taskRemarkText || "").trim();
                if (!nextRemark) {
                    Alert.alert("Error", "Remark is required");
                    setTaskRemarkSaving(false);
                    return;
                }
                await updateCommunicationTaskRemark(
                    taskRemarkTarget.taskId,
                    taskRemarkTarget.remarkId,
                    nextRemark,
                );
                await loadTasks();
                ok = true;
            } else {
                if (!taskRemarkTarget?.status) {
                    setTaskRemarkSaving(false);
                    return;
                }
                ok = await setTaskStatus(
                    taskRemarkTarget.taskId,
                    taskRemarkTarget.status,
                    taskRemarkText,
                );
            }
            if (ok) {
    ...
    }, [loadTasks, setTaskStatus, taskRemarkTarget, taskRemarkText]);
    ...
    function renderTaskRemarkModal() {
        if (!showTaskRemarkModal || !taskRemarkTarget) return null;
        return (
            <Modal
                visible={showTaskRemarkModal}
                transparent
                statusBarTranslucent
                animationType="fade"
                ...
                <View style={S.taskRemarkOverlay}>
                    <TouchableOpacity
                        ...
                        activeOpacity={1}
                        onPress={closeTaskRemarkModal}
                    />
                    <KeyboardAvoidingView
                        style={S.taskRemarkKav}
                        behavior={Platform.OS === "ios" ? "padding" : "height"}
                        keyboardVerticalOffset={
                            Platform.OS === "ios" ? insets.top + 8 : 0
                        }>
                        <View style={S.taskRemarkCard}>
                            <Text style={S.taskRemarkTitle}>
                                {taskRemarkTarget.mode === "editRemark"
                                    ? "Edit Remark"
                                    : "Add Remark"}
                            </Text>
                            <Text style={S.taskRemarkSubTitle}>
                                {taskRemarkTarget.mode === "editRemark"
                                    ? `${taskRemarkTarget.title} • Edit Remark`
                                    : `${taskRemarkTarget.title} • ${taskRemarkTarget.status}`}
                            </Text>
                            <TextInput
                                style={S.taskRemarkInput}
                                placeholder="Write your remark..."
                                placeholderTextColor={T.mute}
                                multiline
                                textAlignVertical="top"
                                autoFocus
                                editable={!taskRemarkSaving}
                                value={taskRemarkText}
                                onChangeText={setTaskRemarkText}
                            />
                            <View style={S.taskRemarkBtnRow}>
                                <TouchableOpacity
                                    style={[S.taskAction, S.taskActionGhost]}
                                    disabled={taskRemarkSaving}
                                    onPress={closeTaskRemarkModal}>
                                ...
                                <TouchableOpacity
                                    style={[S.taskAction, S.taskActionPrimary]}
                                    disabled={taskRemarkSaving}
                                    onPress={submitTaskRemarkUpdate}>
                                    {taskRemarkSaving ? (
                                        <ActivityIndicator
                                            size="small"
                                            color="#fff"
    ...
    function renderTaskDetailModal() {
        ...
        const attachmentSource = item.attachmentUrl
            ? getImageUrl(item.attachmentUrl)
            : "";
        const taskRemarks = getTaskRemarkEntries(item);
                            ...
                            <ScrollView
                                ...
                                <View style={S.detailTaskCard}>
                                    ...
                                    <View style={S.taskInner}>
                                        ...
                                        {taskRemarks.length ? (
                                            <View style={S.taskRemarkBlock}>
                                                <Text style={S.taskRemarkLabel}>
                                                    Remarks History
                                                </Text>
                                                {taskRemarks.map((row, idx) => {
                                                    const canManageRemark = canManageTaskRemark(row);
                                                    return (
                                                        <View key={`${row.updatedAt || "na"}-${idx}`} style={S.taskRemarkRow}>
                                                            <View style={S.taskRemarkRowTop}>
                                                                <View
                                                                    style={[
                                                                        S.taskRemarkDot,
                                                                        { backgroundColor: remarkStatusTone(row.status).dot },
                                                                    ]}
                                                                />
                                                                <View style={S.taskRemarkMetaWrap}>
                                                                    <View
                                                                        style={[
                                                                            S.taskRemarkStatusChip,
                                                                            {
                                                                                backgroundColor: remarkStatusTone(row.status).bg,
                                                                                borderColor: remarkStatusTone(row.status).border,
                                                                            },
                                                                        ]}>
                                                                        <Text
                                                                            style={[
                                                                                S.taskRemarkStatusText,
                                                                                { color: remarkStatusTone(row.status).text },
                                                                            ]}>
                                                                            {row.status || "Updated"}
                                                                        </Text>
                                                                    </View>
                                                                    <Text style={S.taskRemarkMeta}>
                                                                        {row.updatedBy ? row.updatedBy : "Team"}
                                                                        {row.updatedAt ? ` | ${formatRemarkTime(row.updatedAt)}` : ""}
                                                                    </Text>
                                                                </View>
                                                                {canManageRemark ? (
                                                                    <View style={S.taskRemarkActions}>
                                                                        <TouchableOpacity
                                                                            style={S.taskRemarkActionBtn}
                                                                            onPress={() => openEditTaskRemarkModal(item, row)}>
                                                                            <Ionicons name="create-outline" size={13} color={T.mid} />
                                                                        </TouchableOpacity>
                                                                        <TouchableOpacity
                                                                            style={S.taskRemarkActionBtn}
                                                                            onPress={() => deleteTaskRemark(item, row)}>
                                                                            <Ionicons name="trash-outline" size={13} color={T.danger} />
                                                                        </TouchableOpacity>
                                                                    </View>
                                                                ) : null}
                                                            </View>
                                                            <Text style={S.taskRemarkValue}>{row.remark}</Text>
                                                        </View>
                                                    );
                                                })}
                                            </View>
    ...
    return (
        <SafeAreaView style={S.screen} edges={["left", "right"]}>
            ...
            <View style={[S.listHeader, { paddingTop: insets.top + 10, paddingHorizontal: 12, paddingBottom: 14 }]}>
                <TouchableOpacity
            ...
            {tab === "Chats" ? (
                <>
                    ...
                    {/* My Work personal workload section */}
                    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#F9FAFB", paddingHorizontal: 12, paddingVertical: 8, marginHorizontal: 10, marginBottom: 8, borderRadius: 10, borderWidth: 0.8, borderColor: "#E5E7EB" }}>
                        <Text style={{ fontSize: 13, fontWeight: "700", color: "#374151" }}>My Work</Text>
                        <View style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
                            <View style={{ backgroundColor: "#F9FAFB", borderColor: "#E5E7EB", borderWidth: 0.8, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 1.5 }}>
                                <Text style={{ fontSize: 10.5, fontWeight: "800", color: "#6B7280" }}>P {selfTaskCounts.pending}</Text>
                            </View>
                            <View style={{ backgroundColor: "#FFFBEB", borderColor: "#FDE68A", borderWidth: 0.8, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 1.5 }}>
                                <Text style={{ fontSize: 10.5, fontWeight: "800", color: "#D97706" }}>I {selfTaskCounts.inProgress}</Text>
                            </View>
                            <View style={{ backgroundColor: "#ECFDF5", borderColor: "#A7F3D0", borderWidth: 0.8, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 1.5 }}>
                                <Text style={{ fontSize: 10.5, fontWeight: "800", color: "#10B981" }}>C {selfTaskCounts.completed}</Text>
                            </View>
                        </View>
                    </View>
                    <ScrollView
