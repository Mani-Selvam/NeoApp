...
export default function RazorpayCheckoutScreen({ navigation, route }) {
    ...
    const onMessage = useCallback(
        async (msg) => {
            ...
            if (msg.type === "error") {
                Alert.alert("Payment Error", msg.message || "Payment initialization failed");
                navigation.goBack();
                return;
            }
            ...
            if (msg.type === "success") {
                ...
                try {
                    ...
                    Alert.alert("Payment Verification Failed", e?.message || "Unable to verify payment");
                    navigation.goBack();
                } finally {
                    setVerifying(false);
                }
...
<html>
  ...
  <body>
    ...
    }, [amountInr, amountInrPaise, keyId, notes, orderId, prefill.contact, prefill.email, prefill.name, theme.color]);
