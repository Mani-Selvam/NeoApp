...
const isCallActivity = (item) =>
    String(item?.activityType || item?.type || "")
        .toLowerCase()
        .includes("call");
...
function FollowUpNotesPanel({ enquiry, noteRows = [], onSaved }) {
    // Open by default based on feedback
    ...
    return (
        ...
        >
            {/* Add Note button - shown when form is hidden */}
            {!showForm && (
                <TouchableOpacity
                    onPress={() => setShowForm(true)}
                    style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#2563EB", padding: 14, borderRadius: 10, elevation: 2 }}
                    activeOpacity={0.85}
                >
                    <Ionicons name="add-circle-outline" size={20} color="#fff" style={{ marginRight: 8 }} />
                    <Text style={{ color: "#fff", fontWeight: "700", fontSize: 15 }}>Add Note</Text>
                </TouchableOpacity>
            )}

            {/* Collapsible form - shown when Add Note is clicked */}
            {showForm && (
                <View style={{ backgroundColor: "#FFFFFF", padding: 16, borderRadius: 12, elevation: 3, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4, borderWidth: 1, borderColor: "#DBEAFE" }}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                        <Text style={{ fontSize: 16, fontWeight: "700", color: "#0F172A" }}>📝 New Note</Text>
                    ...
                    <View style={{ marginTop: 14 }}>
                        <Text style={{ fontSize: 13, fontWeight: "600", color: "#334155", marginBottom: 8 }}>Voice Note</Text>
                        {!recordedURI ? (
                            <TouchableOpacity
                                onPress={recording ? stopRecording : startRecording}
                                style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: recording ? "#FEE2E2" : "#EFF6FF", padding: 11, borderRadius: 8, borderWidth: 1, borderColor: recording ? "#FCA5A5" : "#BFDBFE" }}
                            >
                            ...
                            <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: "#F0FDF4", padding: 11, borderRadius: 8, borderWidth: 1, borderColor: "#86EFAC" }}>
                                <TouchableOpacity onPress={isPlaying ? stopSound : playSound} style={{ padding: 7, backgroundColor: "#DCFCE7", borderRadius: 20 }}>
                                    <Ionicons name={isPlaying ? "pause" : "play"} size={20} color="#059669" />
                                </TouchableOpacity>
                                <Text style={{ flex: 1, marginLeft: 10, color: "#065F46", fontWeight: "500" }}>Audio Recorded</Text>
                                <TouchableOpacity onPress={clearAudio} style={{ padding: 7 }}>
                    ...
                    <View style={{ flexDirection: "row", gap: 10, marginTop: 20 }}>
                        <TouchableOpacity
                            ...
                            style={{ flex: 1, padding: 13, borderRadius: 8, borderWidth: 1, borderColor: "#CBD5E1", alignItems: "center" }}
                        >
                        ...
                        <TouchableOpacity
                            ...
                            style={{ flex: 2, backgroundColor: (!note.trim() && !recordedURI) ? "#94A3B8" : "#2563EB", padding: 13, borderRadius: 8, flexDirection: "row", alignItems: "center", justifyContent: "center" }}
                        >
                            {isSaving ? (
                ...
                <View style={{ marginTop: 8 }}>
                    ...
                    {noteRows.map((n, i) => (
                        ...
                        >
                            ...
                            {n.voiceNote ? (
                                <VoiceNotePlayer uri={n.voiceNote} />
                            ) : null}
                        </View>
                    ))}
                </View>
            ...
            {/* Empty state prompt when no notes exist */}
            {noteRows && noteRows.length === 0 && !showForm && (
                <View style={{ alignItems: "center", marginTop: 32, paddingHorizontal: 20, opacity: 0.75 }}>
                    <Text style={{ fontSize: 40 }}>🗒️</Text>
                    <Text style={{ fontSize: 15, fontWeight: "700", color: "#334155", marginTop: 10 }}>No notes yet</Text>
                    <Text style={{ fontSize: 13, color: "#64748B", marginTop: 6, textAlign: "center", lineHeight: 20 }}>{`Tap Add Note above to record\nkey details, budget, preferences\nor attach a voice recording.`}</Text>
                </View>
            )}
        </ScrollView>
    );
            ...
            <View style={DV.emailHero}>
                ...
                <TouchableOpacity
                    ...
                    <Text style={DV.emailToggleText}>
                        {showComposer ? "Close Form" : "Sent Email"}
                    </Text>
                </TouchableOpacity>
            </View>
...
const getHistoryEditStatus = (item = {}) => {
    // Notes are internal — never affect the displayed enquiry status
    const actType = String(item?.activityType || item?.type || "").trim();
    if (actType === "Note") return "Contacted";
...
const DetailView = ({
    ...
    const timelineRows = useMemo(
        () =>
            [...(Array.isArray(history) ? history : [])]
                .filter(r => r.type !== "Note" && r.activityType !== "Note")
                .sort(
    ...
    const noteRows = useMemo(
        () =>
            [...(Array.isArray(history) ? history : [])]
                .filter(r => r.type === "Note" || r.activityType === "Note")
                .sort(
                    (a, b) => getHistorySortTs(b) - getHistorySortTs(a), // Newest first
                ),
        [history],
    );
    ...
    useEffect(() => {
        // Prevent closing the form if we were explicitly commanded to auto-open it
        if (!autoOpenFollowUpFormToken) {
            setShowFollowUpForm(false);
        }
    }, [enquiry?._id, enquiry?.enqNo, autoOpenFollowUpFormToken]);
    useEffect(() => {
                ...
                <View style={{ flex: 1, minHeight: 0 }} {...detailPanHandlers}>
                    ...
                    {tabIdx === 3 && (
                        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
                            <Text style={DV.panelEyebrow}>Call Records</Text>
                            {callRows.map((h, i) => {
                                const dateStr = h.activityTime || h.createdAt || h.dueAt || h.date;
                                let formattedDate = "-";
                                let formattedTime = "-";
                                if (dateStr) {
                                ...
                                const contactName = enquiry?.name || selectedEnquiry?.name || "Contact";
                                const itemLabel = h.remarks || "Phone Call";
                                const avatarImage = h.image || enquiry?.image || selectedEnquiry?.image;
                                return (
                                    <TouchableOpacity
                                        key={h._id || i}
                                        activeOpacity={0.85}
                                        onPress={onStartCall}
                                        style={[FU.timelineCard, { paddingVertical: 14, paddingHorizontal: 14 }]}
                                    >
                                        <View style={FU.timelineIconBox}>
                                            {avatarImage ? (
                                                <Image
                                                    source={{ uri: getImageUrl(avatarImage) }}
                                                    style={{ width: "100%", height: "100%", borderRadius: 24 }}
                                                />
                                            ) : (
                                                <Text style={FU.timelineInitials}>
                                                    {getInitials(contactName)}
                                                </Text>
                                            )}
                                        </View>
                                        <View style={FU.timelineBody}>
                                            <View style={FU.timelineRowTop}>
                                                <Text style={FU.timelineTitle}>{contactName}</Text>
                                                <Ionicons name="call" size={16} color={C.primary} />
                                            </View>
                                            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 6 }}>
                                                <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: C.primarySoft, borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3 }}>
                                                    <Ionicons name="calendar-outline" size={11} color={C.primary} />
                                                    <Text style={{ color: C.primaryDark, fontSize: 11, fontWeight: "800" }}>
                                                        {formattedDate}
                                                    </Text>
                                                </View>
                                                <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#ECFDF5", borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3 }}>
                                            ...
                                            <Text style={FU.timelineNote}>{itemLabel}</Text>
                                        </View>
                                    </TouchableOpacity>
                                );
                            })}
                            {callRows.length === 0 && (
                                <View style={FU.timelineEmpty}>
                                    <Ionicons name="call-outline" size={48} color={C.border} />
                                    <Text style={FU.timelineEmptyText}>No call records yet.</Text>
                                </View>
                            )}
                        </ScrollView>
                    )}
                                    ...
                                    <View style={{ gap: 12 }}>
                                        ...
                                        <View style={FU.sectionCard}>
                                            ...
                                            {historyLoading ? (
                                                ...
                                                />
                                            ) : timelineRows.filter(h => !(isCallActivity(h) && h.status === 'Completed')).length === 0 ? (
                                                <View style={FU.timelineEmpty}>
                                                    <Ionicons
                                                        name="time-outline"
                                                        size={20}
...
export default function FollowUpScreen({ navigation, route }) {
    ...
    const handleStartContactCall = useCallback(async (enquiry) => {
        ...
        const recordCall = async () => {
            const now = new Date();
            const rawAT = enquiry?.assignedTo?._id || enquiry?.assignedTo;
            const atId = typeof rawAT === "string" ? rawAT : "";
            try {
                await followupService.createCallRecord({
                    enqId: enquiry?._id || enquiry?.enqId,
                    enqNo: enquiry?.enqNo,
                    name: enquiry?.name,
                    mobile: enquiry?.mobile,
                    product: enquiry?.product,
                    image: enquiry?.image,
                    ...(atId ? { assignedTo: atId } : {}),
                    remarks: "Outgoing phone call",
                    activityTime: now.toISOString(),
                });
                await Promise.all([
                    Promise.resolve(refreshDetailHistory?.()),
                    Promise.resolve(refreshDetailEnquiry?.()),
                ...
                console.warn(
                    "[FollowUpScreen] Call started, but call record was not saved",
                    logError?.response?.data || logError?.message || logError,
                );
            }
        };
        ...
        try {
            ...
            if (Platform.OS === "android") {
                ...
                if (granted) {
                    ...
                    if (immediateCall?.immediatePhoneCall) {
                        try {
                            immediateCall.immediatePhoneCall(digits);
                            await recordCall();
                            return;
                        } catch (directErr) {
            ...
            // Safe mode/iOS fallback: open the system dialer.
            await Linking.openURL(`tel:${digits}`);
            await recordCall();
        } catch (_error) {
