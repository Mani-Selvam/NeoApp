...
export default function AddEnquiryScreen({ route, navigation }) {
    ...
    // Form
    const [playStoreSafeMode, setPlayStoreSafeMode] = useState(false);
    ...
    useEffect(() => {
        ...
        const fetchConfig = async () => {
            try {
                ...
                if (isMounted && res.data && typeof res.data.playStoreSafeMode === "boolean") {
                    setPlayStoreSafeMode(res.data.playStoreSafeMode);
                }
            } catch (err) {
                console.warn("Failed to fetch server config", err?.message);
            }
        };
        fetchConfig();
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        if (!isEditMode && data?._id) {
                            // After creating a new enquiry, navigate to FollowUp screen
                            // and auto-open the Add Follow-up form for the new lead
    ...
    const filteredDeviceContacts = useMemo(() => {
        ...
        return deviceContacts.filter(c =>
            ...
            (c.phoneNumbers && c.phoneNumbers.some(p => p.number && p.number.replace(/\D/g, '').includes(s)))
        );
    ...
    const handleOpenContacts = async () => {
        try {
            ...
            if (status !== 'granted') {
                Alert.alert("Permission Required", "Please grant contacts permission to pick a number.");
                return;
            }
                ...
                for (const c of data) {
                    if (c.phoneNumbers && c.phoneNumbers.length > 0) {
                        ...
                        for (const p of c.phoneNumbers) {
                            ...
                            let digits = p.number.replace(/\D/g, '');
                            if (digits.length >= 10) {
                                if (digits.length > 10) digits = digits.slice(-10);
                                if (!uniqueNumbers.has(digits)) {
                        ...
                        if (uniquePhones.length > 0) {
                            valid.push({ ...c, phoneNumbers: uniquePhones });
                        }
                    }
                }
    ...
    const handleSelectContact = (contact, phoneObj) => {
        let number = phoneObj.number.replace(/\D/g, "");
        if (number.length > 10) number = number.slice(-10);
        setForm(p => ({ ...p, mobile: number, name: p.name || contact.name || "" }));
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
                        ...
                        <Animated.View
                                ...
                                {!isStaffUser && assignableTeam.length > 0 && (
                                    <DropBtn
                                        ...
                                        value={
                                            form.assignedTo
                                                ? (() => {
                                                    const s = assignableTeam.find((s) => s._id === form.assignedTo);
                                                    if (!s) return "Unknown";
                                                    const roleLabel = String(s.role || "").toLowerCase() === "admin"
                                        ...
                                        placeholder={`${user?.name || "Me"} (${String(user?.role || "").toLowerCase() === "admin" ? (adminLabelMap[String(user?._id || user?.id)] || "Admin") : "Staff"}) (Auto-assign)`}
                            ...
                            <Card icon="person-outline" title="Contact" sc={sc}>
                                ...
                                <Field
                                    ...
                                    error={errors.mobile}
                                    rightIcon={!playStoreSafeMode ? "book" : undefined}
                                    onRightIconPress={() => {
                                        if (showContactModal) setShowContactModal(false);
                                        else handleOpenContacts();
                                    }}
                                />
                                {showContactModal && (
                                    <View style={{
                                        ...
                                        <View style={{ padding: sc.sp.sm, borderBottomWidth: 1, borderColor: C.border, backgroundColor: C.cardAlt }}>
                                            <View style={{
                                                flexDirection: "row",
                                                alignItems: "center",
                                                backgroundColor: C.card,
                                                ...
                                                borderWidth: 1,
                                                borderColor: C.primaryMid,
                                                paddingHorizontal: sc.sp.sm,
                                                height: sc.inputH * 0.85,
                                            }}>
                                                <Ionicons name="search" size={sc.f.md} color={C.textLight} style={{ marginRight: sc.sp.xs }} />
                                                <TextInput
                                                    style={{
                                                        flex: 1,
                                                        fontSize: sc.f.sm,
                                                ...
                                                {contactSearch.length > 0 && (
                                                    <TouchableOpacity onPress={() => setContactSearch("")}>
                                                        <Ionicons name="close-circle" size={sc.f.md} color={C.textLight} />
                                                    </TouchableOpacity>
                                                )}
                                            </View>
                                        </View>
                                        {isFetchingContacts ? (
                                            <View style={{ padding: 20, alignItems: "center" }}><ActivityIndicator color={C.primary} /></View>
                                        ) : (
                                            <ScrollView style={{ flexGrow: 1 }} contentContainerStyle={{ paddingBottom: sc.sp.lg }} keyboardShouldPersistTaps="handled" nestedScrollEnabled={true}>
                                                {filteredDeviceContacts.slice(0, 50).map((item) => {
                                                    if (item.phoneNumbers && item.phoneNumbers.length === 1) {
                                                        return (
                                                            <TouchableOpacity
                                                                key={item.id}
                                                                onPress={() => handleSelectContact(item, item.phoneNumbers[0])}
                                                                style={{ borderBottomWidth: 1, borderColor: C.border, padding: sc.sp.sm, width: "100%" }}>
                                                                <Text style={{ fontSize: sc.f.sm, fontWeight: "600", color: C.text, marginBottom: 4 }}>{item.name}</Text>
                                                                <View style={{ flexDirection: "row", alignItems: "center" }}>
                                                                    <Ionicons name="call" size={14} color={C.primary} style={{ marginRight: 6 }} />
                                                                    <Text style={{ fontSize: sc.f.sm, color: C.textSub }}>{item.phoneNumbers[0].number}</Text>
                                                                </View>
                                                            </TouchableOpacity>
                                                        );
                                                    }
                                                    return (
                                                        <View key={item.id} style={{ borderBottomWidth: 1, borderColor: C.border }}>
                                                            <Text style={{ fontSize: sc.f.sm, fontWeight: "600", color: C.text, paddingTop: sc.sp.sm, paddingHorizontal: sc.sp.sm, marginBottom: 2 }}>{item.name}</Text>
                                                            {item.phoneNumbers && item.phoneNumbers.map((p, idx) => (
                                                                <TouchableOpacity
                                                                    key={idx}
                                                                    onPress={() => handleSelectContact(item, p)}
                                                                    style={{ flexDirection: "row", alignItems: "center", paddingVertical: 10, paddingHorizontal: sc.sp.sm, width: "100%" }}>
                                                                    <Ionicons name="call" size={14} color={C.primary} style={{ marginRight: 6 }} />
                                                                    <Text style={{ fontSize: sc.f.sm, color: C.textSub }}>{p.number}</Text>
                                                                </TouchableOpacity>
                                                            ))}
                                                        </View>
                                                    );
            ...
            <SelectModal
                ...
                <View
                    ...
                    <ChoiceCard
                        label={`${user?.name || "Me"} (Auto-assign)`}
                        sublabel={String(user?.role || "").toLowerCase() === "admin" ? (adminLabelMap[String(user?._id || user?.id)] || "Admin") : "Assign to yourself"}
                        icon="person-outline"
