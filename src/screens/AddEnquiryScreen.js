...
export default function AddEnquiryScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (isEditMode && editingEnquiry) {
      setForm((prev) => ({
        ...
        cost: editingEnquiry.cost ? String(editingEnquiry.cost) : prev.cost,
  ...
  const handleAddressChange = (text) => {
    ...
    if (text.trim().length >= 3) {
      ...
      addressTimeoutRef.current = setTimeout(async () => {
        try {
          const predictions = await addressService.getAddressPredictions(text);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (submissionData.assignedTo === "") submissionData.assignedTo = null;
      ...
      if (response.ok) {
        ...
        await notificationService.showEnquiryErrorNotification(errorMessage);
                  ...
                  >
                    <View style={styles.dropdownLeft}>
                      ...
                      <View>
                        ...
                        >
                          {form.assignedTo
                            ? staffList.find((s) => s._id === form.assignedTo)
                                ?.name || "Unknown Staff"
                            : "Me (Auto-assign)"}
                        </Text>
                        ...
                        {!form.assignedTo && (
                          ...
                          >
                            Will be assigned to you automatically
                          </Text>
                        )}
                      </View>
                    </View>
              ...
              <View style={styles.modalGrid}>
                {products.map((prod) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.sourceCard,
                      form.product === prod.name && styles.sourceCardActive,
                    ]}
                    onPress={() => {
            ...
            {loadingStaff ? (
              ...
              <ScrollView style={styles.modalScroll}>
                <TouchableOpacity
                  style={[
                    styles.sourceCard,
                    !form.assignedTo && styles.sourceCardActive,
                ...
                >
                  ...
                  >
                    <Ionicons
                      name={
                        !form.assignedTo ? "checkmark-circle" : "person-outline"
                      }
                      size={22}
                      color={
                        !form.assignedTo ? COLORS.primary : COLORS.gray[400]
                      }
                    />
                  </View>
                  <View>
                    <Text
                      style={[
                        styles.sourceName,
                        !form.assignedTo && { color: COLORS.white },
                      ]}
                    >
                      Me (Auto-assign)
                    </Text>
                    <Text
                      style={[
                        styles.sourceGroup,
                        !form.assignedTo && { color: "rgba(255,255,255,0.8)" },
                      ]}
                    >
                      Assign to yourself
                    </Text>
                ...
                <View style={styles.modalGrid}>
                  {staffList.map((staff) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        updateField("assignedTo", staff._id);
                        setShowStaffModal(false);
                      }}
                    >
                      <View
