...
const getStaffName = (item, adminName = "Admin", staffNameById = {}) => {
    const directName =
        item?.staffName ||
        item?.staffId?.name ||
        ...
        item?.enqBy ||
        "";
    const normalizedDirect = normalizeStaffLabel(directName, "");
    if (normalizedDirect) return normalizedDirect;

    const id =
        normalizeId(item?.assignedTo?._id || item?.assignedTo) ||
        normalizeId(item?.staffId?._id || item?.staffId) ||
        normalizeId(item?.createdBy?._id || item?.createdBy) ||
        normalizeId(item?.assignedToId);
    const mapped = id ? normalizeStaffLabel(staffNameById[id], "") : "";
    return mapped || adminName;
};
...
const getItemStaffIds = (item) =>
    [
        normalizeId(item?.assignedTo?._id || item?.assignedTo),
        normalizeId(item?.staffId?._id || item?.staffId),
        normalizeId(item?.createdBy?._id || item?.createdBy),
        normalizeId(item?.assignedToId),
        normalizeId(item?.enqId?.assignedTo?._id || item?.enqId?.assignedTo),
        normalizeId(
            item?.enquiryId?.assignedTo?._id || item?.enquiryId?.assignedTo,
        ),
    ].filter(Boolean);

const matchesStaffFilter = (
    item,
    staffFilter,
    adminName,
    staffNameById,
    ...
    if (staffFilter === ALL_STAFF) return true;
    const selected = String(staffFilter || "").trim();
    if (!selected) return true;

    const itemIds = getItemStaffIds(item);
    const selectedIds = Array.isArray(staffIdsByName[selected])
        ? staffIdsByName[selected]
        : [];
    if (
    ...
    const candidateNames = Array.from(
        new Set(
            [
                ...
                item?.user?.name,
                item?.enqBy,
                ...itemIds.map((id) => staffNameById[id]),
            ]
                .map((n) => normalizeStaffLabel(n, ""))
                .filter(Boolean),
        ),
    );

    return candidateNames.includes(selected);
};
...
const ActionArrowCue = ({ color = C.textMuted }) => {
    ...
    return (
        <View style={st.rowActionCue}>
            <Text style={[st.rowActionText, { color }]}>View</Text>
...
export default function ReportScreen({ navigation }) {
    ...
    const { user, logout, billingInfo } = useAuth();
    ...
    const [draftFromDate, setDraftFromDate] = useState(monthStartIso);
        ...
        async ({ force = false, showLoading = true } = {}) => {
            ...
            try {
                ...
                if (cached?.value) {
                    ...
                    if (Array.isArray(cached.value?.staffDirectory)) {
                        setStaffDirectory(cached.value.staffDirectory);
                    }
                    usedCache = true;
    ...
    useEffect(() => {
        debounceByKey(
            "report-range-refresh",
            () => loadReportData({ force: true, showLoading: false }),
            250,
        );
        return () => cancelDebounceKey("report-range-refresh");
    }, [fromDate, toDate, loadReportData]);
    ...
    const staffIdsByName = useMemo(() => {
        const map = {};
        const add = (name, id) => {
            const n = normalizeStaffLabel(name, "");
            const k = normalizeId(id);
            if (!n || !k) return;
            if (!Array.isArray(map[n])) map[n] = [];
    ...
    const staffOptions = useMemo(() => {
        ...
        const uniqueStaff = Array.from(
            new Set([...fromDirectory, ...fromReportRows]),
    ...
    const staffPerf = useMemo(() => {
        ...
        const map = {};
        const resolveStaffId = (name) =>
            Array.isArray(staffIdsByName[name]) ? staffIdsByName[name][0] : "";
        const ensure = (name) => {
            const n = normalizeStaffLabel(name, adminName);
            if (!map[n])
                map[n] = {
                    name: n,
                    staffId: resolveStaffId(n),
                    enquiriesCreated: 0,
                    followupsDone: 0,
                    salesLeads: 0,
                };
            if (!map[n].staffId) map[n].staffId = resolveStaffId(n);
            return map[n];
        };

        const baseNames = (Array.isArray(staffDirectory) ? staffDirectory : [])
            .map((item) => normalizeStaffLabel(item?.name, ""))
            .filter(Boolean);
        if (staffFilter === ALL_STAFF) {
            baseNames.forEach((name) => ensure(name));
        } else {
            ensure(staffFilter);
        }
        ...
        filteredEnq.forEach((i) => {
            ...
            const assigneeName = getStaffName(i, creator, staffNameById);
        ...
        // Followups done: resolved by best available name source.
        filteredFups.forEach((i) => {
            const actor = getStaffName(i, adminName, staffNameById);
    ...
    const openStaffYearReport = useCallback(
        (rowOrName, maybeStaffId = "") => {
            const staffName = String(
                typeof rowOrName === "object" && rowOrName !== null
                    ? rowOrName?.name
                    : rowOrName,
            ).trim();
            const staffId = String(
                typeof rowOrName === "object" && rowOrName !== null
                    ? rowOrName?.staffId || maybeStaffId
                    : maybeStaffId,
            ).trim();
            if (!staffName) return;

            const params = staffId ? { staffName, staffId } : { staffName };
            // StaffPerformanceReport lives inside MainTabNavigator.
            try {
                let nav = navigation;
    ...
    const exportReport = async () => {
        try {
            ...
            const leadRows = filteredEnq.map((item) => ({
                ...
                Staff: getStaffName(item, adminName, staffNameById),
    ...
    return (
        <SafeAreaView style={st.container} edges={["top"]}>
            ...
            <View
                style={[st.topHeader, { paddingTop: insets.top > 0 ? 8 : 14, paddingHorizontal: 12, paddingBottom: 14 }]}>
                <TouchableOpacity
