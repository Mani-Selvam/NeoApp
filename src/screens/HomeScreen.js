...
const addDaysIso = (iso, days) => {
    const dt = isoToDate(iso) || new Date();
...
export default function HomeScreen({ navigation }) {
    ...
    // Responsive values from global contract
    const ui = useResponsiveTokens();
    const { width: w, isTablet, isSmallScreen } = ui;
    const isDesktop = false;
    const isSmallPhone = isSmallScreen;
    ...
    useEffect(() => {
        ...
        const unsubEnquiryCreated = onAppEvent(
            APP_EVENTS.ENQUIRY_CREATED,
            refreshDashboard,
        );
        return () => {
    ...
    const missedSections = useMemo(() => {
        const list = Array.isArray(missedTasks) ? missedTasks : [];
        ...
        list.forEach((item) => {
            ...
            if (d === todayIso) buckets.today.push(item);
            else if (d === yesterdayIso) buckets.yesterday.push(item);
        ...
        const ordered = [
            {
                key: "today",
                title: "Today",
                dateLabel: fmtLongDate(todayIso),
                items: buckets.today.sort(sortByNewest),
            },
            {
                key: "yesterday",
                title: "Yesterday",
                dateLabel: fmtLongDate(yesterdayIso),
                items: buckets.yesterday.sort(sortByNewest),
            },
            {
                key: "earlier",
                title: "Earlier Missed",
                dateLabel: "Before yesterday",
                items: buckets.earlier.sort(sortByNewest),
            },
        ];
    ...
    const missedActivityCount = Number(stats.missedFollowup || 0);

    const openMissedEnquiryDetail = (item) => {
        const enqNo = String(item?.enqNo || "").trim();
        const enqId = String(item?.enqId || "").trim();
        if (!enqNo && !enqId) return;
        setMissedListOpen(false);
        navigation.navigate("FollowUp", {
            openComposer: true,
            composerToken: `home-missed-${enqNo || enqId}-${Date.now()}`,
            enquiry: {
                enqId: enqId || null,
                enqNo,
                name: item?.name || "Lead",
            ...
            focusKey: `home-missed-open-${enqNo || enqId}-${Date.now()}`,
        });
    };
                ...
                <View style={S.missedFullScreen}>
                    <View
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => setMissedListOpen(false)}
                            activeOpacity={0.85}>
                            <Ionicons
                                name="chevron-back"
                                size={20}
                        ...
                        <View style={{ flex: 1 }}>
                            <Text style={S.missedTitle}>Missed Activity</Text>
                            <Text style={S.missedSub}>
                                Overall missed follow-ups: {missedActivityCount}
                            </Text>
                        </View>
                    </View>
                    ...
                    <ScrollView
                        ...
                        {missedSections.length > 0 ? (
                            missedSections.map((section) => (
                                <View key={section.key} style={S.missedSection}>
                                    <View style={S.missedSectionHeader}>
                                        <View style={S.missedSectionLeft}>
                                            <View style={S.missedSectionRank}>
                                                <Text
                                                    style={
                                                        S.missedSectionRankText
                                                    }>
                                                    {section.rank}
                                                </Text>
                                            </View>
                                            <View>
                                                <Text
                                                    style={
                                                        S.missedSectionTitle
                                                    }>
                                                    {section.title}
                                                </Text>
                                                <Text
                                                    style={S.missedSectionDate}>
                                                    {section.dateLabel}
                                                </Text>
                                            </View>
                                        </View>
                                        <View style={S.missedSectionCount}>
                                            <Text
                                                style={
                                                    S.missedSectionCountText
                                                }>
                    ...
                    <View
                        ...
                        <MotiView
                            ...
                            <SH
                                ...
                                titleSize={sectionTitleSize}
                                sub="Today follow-ups and overall missed"
                            />
                                ...
                                <View style={S.countRow}>
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            ...
                                            navigation.navigate("FollowUp", {
                                                focusKey: `today-followup-open-${Date.now()}`,
                                                focusTab: "Today",
                                                focusDate: todayIso,
                                            });
                                        }}>
                                    ...
                                    <TouchableOpacity
                                        ...
                                        activeOpacity={0.88}
                                        onPress={() => setMissedListOpen(true)}>
                                        <View
                                            style={[
                                                S.countIconWrap,
...
const S = StyleSheet.create({
    ...
    missedTitle: { fontSize: 18, fontWeight: "900", color: C.ink },
    missedSub: { marginTop: 2, fontSize: 12, color: C.textDim, fontWeight: "600" },
    missedListWrap: {
        paddingHorizontal: 18,
        paddingTop: 14,
        gap: 12,
