...
module.exports = ({ config }) => {
    ...
    const internalAndroidPermissions = [
        ...safeAndroidPermissions,
        "android.permission.CALL_PHONE",
    ];
    const androidPermissions = playStoreSafeMode
        ? safeAndroidPermissions
        : internalAndroidPermissions;

    // These are the specific permission strings to strip out of the manifest for Play Store.
    ...
    return {
        ...
        plugins: (() => {
            ...
            // ─── PLAY STORE SAFETY: Strip expo-contacts ──────────────────────
            // If in safe mode, we must NOT have the contacts plugin as it requests
            // READ_CONTACTS permission which causes Play Store rejection.
            if (playStoreSafeMode) {
                plugins = plugins.filter(
                    (p) => (Array.isArray(p) ? p[0] : p) !== "expo-contacts",
                );
            }
        ...
        extra: {
            ...existingExtra,
            eas: {
                projectId: "022a81e5-d180-43d7-9e1f-e70786a2ea71",
            },
            playStoreSafeMode,
            apiUrl:
