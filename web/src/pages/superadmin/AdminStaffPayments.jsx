...
const AdminStaffPayments = () => {
    ...
    const fetchPayments = async () => {
        try {
            setLoading(true);
            const data = await api.getAdminStaffPayments();
            if (data.success) {
                setPayments(data.payments);
            }
        } catch (error) {
            console.error("Failed to fetch admin/staff payments", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="admin-shell">
            ...
            <div className="admin-main">
                <Header title="Admin / Staff Upgrades" />
                <main className="page-content">
                    <div className="pm-container pm-animate-fade-in pm-glass-panel" style={{ padding: '2rem' }}>
                        <div className="pm-header">
                            <div>
                                <h1 className="pm-title" style={{ fontSize: '1.5rem', fontWeight: 600 }}>👤 Admin & Staff Upgrades</h1>
                                <p className="pm-subtitle" style={{ color: '#6B7280' }}>Track all company purchases for extra Admin and Staff slots.</p>
                            </div>
                                ...
                                <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "1rem" }}>
                                    <thead>
                                        <tr style={{ borderBottom: "1px solid #e5e7eb", textAlign: "left" }}>
                                            ...
                                            <th style={{ padding: "0.75rem", fontWeight: 600, color: '#374151' }}>Qty Added</th>
                                    ...
                                    <tbody>
                                        {payments.map((p) => (
                                            <tr key={p._id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                                                ...
                                                <td style={{ padding: "0.75rem", fontWeight: 600, color: p.type === 'Admin' ? '#8B5CF6' : '#3B82F6' }}>
                                                    {p.type}
                                                </td>
                                                <td style={{ padding: "0.75rem", color: '#4b5563', fontWeight: 600 }}>
                                                    +{p.quantityAdded}
                                                </td>
...
export default AdminStaffPayments;
